...
GreatPeopleDirectiveTypes CvPlayerAI::GetDirectiveWriter(CvUnit* pGreatWriter)
{
...
#ifdef AMB_AI_BETTER_GREAT_PEOPLE_USE
	// If I'm in danger, blast
	...
	// If we're going for culture we want a GW no matter what.
	if(eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && GetDiplomacyAI()->IsGoingForCultureVictory())
	{
		if(GetEconomicAI()->GetBestGreatWorkCity(pGreatWriter->plot(), eGreatWork))
		{
			AMB_LOG("%s turn %i: GW Culture victory, use power",  getName(), GC.getGame().getGameTurn());
			eDirective = GREAT_PEOPLE_DIRECTIVE_USE_POWER;
		}
		//Wait 15 turns for a slot otherwise, blast if its been 15 turn
		else if((GC.getGame().getGameTurn() - pGreatWriter->getGameTurnCreated()) >= GC.getAI_HOMELAND_GREAT_PERSON_TURNS_TO_WAIT())
		{
			AMB_LOG("%s turn %i: GW Culture victory, use blast too long",  getName(), GC.getGame().getGameTurn());
			eDirective = GREAT_PEOPLE_DIRECTIVE_CULTURE_BLAST;
		}
	}
	//Protect against ideology
	else if (eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && GetCulture()->GetPublicOpinionUnhappiness() > 10)
	{
		AMB_LOG("%s turn %i: GW Fight ideology blast",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_CULTURE_BLAST;
	}
	// If not going for culture win and a Level 2 or 3 Tenet is available, try to snag it
	else if (eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && GetPlayerPolicies()->CanGetAdvancedTenet())
	{
		AMB_LOG("%s turn %i: GW buy tenets blast",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_CULTURE_BLAST;
	}
	//If none of that apply make a great work or blast
	else if (eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && GetEconomicAI()->GetBestGreatWorkCity(pGreatWriter->plot(), eGreatWork))
	{
		AMB_LOG("%s turn %i: GW make art",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_USE_POWER;
	}
	else if(eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && (GC.getGame().getGameTurn() - pGreatWriter->getGameTurnCreated()) >= (GC.getAI_HOMELAND_GREAT_PERSON_TURNS_TO_WAIT()-5))
	{
		AMB_LOG("%s turn %i: GW blast too long",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_CULTURE_BLAST;
	}
#else
...
GreatPeopleDirectiveTypes CvPlayerAI::GetDirectiveArtist(CvUnit* pGreatArtist)
{
	...
	// If we're going for culture we want a GW no matter what.
	// If Brazil and we're closing in on Culture Victory we want a golden age
	...
	{
		AMB_LOG("%s turn %i: GA Brazil golden age",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_GOLDEN_AGE;
	}
	if(eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && GetDiplomacyAI()->IsGoingForCultureVictory())
	{
		if(GetEconomicAI()->GetBestGreatWorkCity(pGreatArtist->plot(), eGreatWork))
		{
			AMB_LOG("%s turn %i: GA culture victory make art",  getName(), GC.getGame().getGameTurn());
			eDirective = GREAT_PEOPLE_DIRECTIVE_USE_POWER;
		}
		//Wait 15 turns for a slot otherwise, blast if its been 15 turn
		else if((GC.getGame().getGameTurn() - pGreatArtist->getGameTurnCreated()) >= GC.getAI_HOMELAND_GREAT_PERSON_TURNS_TO_WAIT())
		{
			AMB_LOG("%s turn %i: GA culture victory make art too long",  getName(), GC.getGame().getGameTurn());
			eDirective = GREAT_PEOPLE_DIRECTIVE_GOLDEN_AGE;
		}
	}
	//Ideology pressure and war mobilization are poor reasons to make a golden age so skipped
	//Speed up SS part is a good reason
	else if (eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && GetDiplomacyAI()->IsGoingForSpaceshipVictory() && EconomicAIHelpers::IsTestStrategy_GS_SpaceshipHomestretch(this))
	{
		AMB_LOG("%s turn %i: GA speed up SS",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_GOLDEN_AGE;
	}
	// If Persia and I'm at war, get a Golden Age going
	else if (eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && GetPlayerTraits()->GetGoldenAgeMoveChange() > 0 && GetMilitaryAI()->GetNumberCivsAtWarWith() > 1 && !isGoldenAge())
	{
		AMB_LOG("%s turn %i: GA persia war GA",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_GOLDEN_AGE;
	}
	// If not going for culture win and a Level 2 or 3 Tenet is available, try to snag it
	else if (eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && GetPlayerPolicies()->CanGetAdvancedTenet())
	{
		AMB_LOG("%s turn %i: GA speed tenets",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_GOLDEN_AGE;
	}
	//Great work (before ideologies) or blast otherwise
	// Create Great Work if there is a slot
	else if (eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && GetEconomicAI()->GetBestGreatWorkCity(pGreatArtist->plot(), eGreatWork))
	{
		AMB_LOG("%s turn %i: GA make work",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_USE_POWER;
	}
	else if(eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && (GC.getGame().getGameTurn() - pGreatArtist->getGameTurnCreated()) >= (GC.getAI_HOMELAND_GREAT_PERSON_TURNS_TO_WAIT()-5))
	{
		AMB_LOG("%s turn %i: GA too long GA",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_GOLDEN_AGE;
	}
#else
	// Defend against ideology pressure if not going for culture win
...
GreatPeopleDirectiveTypes CvPlayerAI::GetDirectiveMusician(CvUnit* pGreatMusician)
{
	...
	// If closing in on a Culture win, go for the Concert Tour 500 tourism threshold
	if (eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && GetDiplomacyAI()->IsGoingForCultureVictory() && pGreatMusician->GetTourismBlastStrength() > 2000)
	{		
		CvPlot* pTarget = FindBestMusicianTargetPlot(pGreatMusician, true);
		if(pTarget)
		{
			AMB_LOG("%s turn %i: GM culture victory blast",  getName(), GC.getGame().getGameTurn());
			eDirective = GREAT_PEOPLE_DIRECTIVE_TOURISM_BLAST;
		}
	}
	// Create Great Work if there is a slot otherwise wait for 20 turns max
	else if (eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && GetEconomicAI()->GetBestGreatWorkCity(pGreatMusician->plot(), eGreatWork))
	{
		AMB_LOG("%s turn %i: GM make music",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_USE_POWER;
	}
	else if(eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && (GC.getGame().getGameTurn() - pGreatMusician->getGameTurnCreated()) >= (GC.getAI_HOMELAND_GREAT_PERSON_TURNS_TO_WAIT()+5))
	{
		AMB_LOG("%s turn %i: GM too long blast",  getName(), GC.getGame().getGameTurn());
			eDirective = GREAT_PEOPLE_DIRECTIVE_TOURISM_BLAST;
	}
#else
...
GreatPeopleDirectiveTypes CvPlayerAI::GetDirectiveScientist(CvUnit* /*pGreatScientist*/)
{
	...
	if(eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && !IsSafe(this))
	{
		AMB_LOG("%s turn %i: GS danger !",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_USE_POWER;
	}
#ifdef AMB_AI_BETTER_GREAT_PEOPLE_USE
	//Good AIs finish in 300 turns
	int iTurnLeft=300;
	//adjusting for difficulty
	iTurnLeft+=max(0,20*(5-GC.getGame().getHandicapInfo().GetID()));
	//Speed
	iTurnLeft*=GC.getGame().getGameSpeedInfo().getResearchPercent();
	iTurnLeft/=100;
	iTurnLeft-=GC.getGame().getGameTurn();

	//academy type for functions
	ImprovementTypes eAcademy = (ImprovementTypes)GC.getInfoTypeForString("IMPROVEMENT_ACADEMY");
	//calculate academy value
	int iAcademyValue = GC.getImprovementInfo(eAcademy)->GetYieldChange(YIELD_SCIENCE);
	iAcademyValue += getImprovementYieldChange(eAcademy, YIELD_SCIENCE);
	iAcademyValue += GetPlayerTraits()->GetImprovementYieldChange(eAcademy, YIELD_SCIENCE);
	iAcademyValue += GET_TEAM(getTeam(GetID())).getImprovementYieldChange(eAcademy, YIELD_SCIENCE);
	//with modifiers from capital
	iAcademyValue *= 150+getCapitalYieldRateModifier(YIELD_SCIENCE);
	iAcademyValue /= 100;
	AMB_LOG("Value academy %i",iAcademyValue);
	//bulb value
	int iBulb=GetScience()*5; //to account for extras, future etc so 5 turns instead of 7
	//remove non city stuff
	iBulb*=90;
	iBulb/=100;
	AMB_LOG("Bulb value %i Turn left %i",iBulb,iTurnLeft);
	//can we catch up with an academy
	if((iBulb / iAcademyValue < iTurnLeft)) //eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE && 
	{
		AMB_LOG("%s turn %i: GS make academy !",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_CONSTRUCT_IMPROVEMENT;
	}
	if (eDirective == NO_GREAT_PEOPLE_DIRECTIVE_TYPE)
	{
		// a tech boost is never bad
		AMB_LOG("%s turn %i: GS bulb !",  getName(), GC.getGame().getGameTurn());
		eDirective = GREAT_PEOPLE_DIRECTIVE_USE_POWER;
	}
#else
...
CvPlot* CvPlayerAI::FindBestMusicianTargetPlot(CvUnit* pGreatMusician, bool bOnlySafePaths)
{
	...
	// Find target civ
#ifdef AMB_AI_BETTER_GREAT_PEOPLE_USE
	//musician can enter territories now
	PlayerTypes eTargetPlayer = GetCulture()->GetCivLowestInfluence(false /*bCheckOpenBorders*/);
#else
