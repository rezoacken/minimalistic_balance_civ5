...
void CvUnitCombat::GenerateMeleeCombatInfo(CvUnit& kAttacker, CvUnit* pkDefender, CvPlot& plot, CvCombatInfo* pkCombatInfo)
{
	...
	if(plot.isCity())
	{
		...
		//iDefenderStrength *= (100 - AMB_CITY_WEAKER_RANGE);
...
void CvUnitCombat::ResolveMeleeCombat(const CvCombatInfo& kCombatInfo, uint uiParentEventID)
{
	        ...
	        pkDefender->IsCanDefend()) 		// Did the defender actually defend?
	{
		...
		if(bAttackerDead)
		{
			...
			pkAttacker->DoTestWarmongerThreatToMinorsWithThisUnitsDeath(pkDefender->getOwner());
#endif
		}
		else if(bDefenderDead)
		{
...
#ifdef AMB_DEFEND_CS_INFLUENCE
			pkDefender->DoTestWarmongerThreatToMinorsWithThisUnitsDeath(pkAttacker->getOwner());
#endif
		}
	}
...
#ifdef AMB_WEAKER_NUKES
								if(iDamageLevel == 1)	// Nuke level 1
								{
									iNukeDamage = (/*50*/ GC.getNUKE_UNIT_DAMAGE_BASE() + /*25*/ GC.getGame().getJonRandNum(GC.getNUKE_UNIT_DAMAGE_RAND_1(), "Nuke Damage 1"));
								}
								// Wipe everything out
								else
								{
									iNukeDamage = (/*50*/ GC.getNUKE_UNIT_DAMAGE_BASE() + /*25*/ GC.getGame().getJonRandNum(GC.getNUKE_UNIT_DAMAGE_RAND_1(), "Nuke Damage 1") + /*35*/ GC.getGame().getJonRandNum(GC.getNUKE_UNIT_DAMAGE_RAND_2(), "Nuke Damage 2"));
								}
#else
