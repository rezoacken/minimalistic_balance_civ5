...
#ifdef AMB_AI_TREASURY_MANAGEMENT
	m_iMoneyForRushs = 0;
	m_iMoneyForPlots = 0;
	m_iMoneyForMinorGifts = 0;
	m_iMoneyForUnitUpgrades = 0;
	//m_iPreviousTreasury = 0;
#endif
	// Cached AI defines
...
#ifdef AMB_AI_TREASURY_MANAGEMENT
	kStream >> m_iMoneyForRushs;
	kStream >> m_iMoneyForPlots;
	kStream >> m_iMoneyForMinorGifts;
	kStream >> m_iMoneyForUnitUpgrades;
	//kStream >> m_iPreviousTreasury;
#endif
...
#ifdef AMB_BETTER_POLICY_STRATEGY
				else if(strStrategyName == "ECONOMICAISTRATEGY_RELIGIOUS_CIV")
					bStrategyShouldBeActive = EconomicAIHelpers::IsTestStrategy_IsReligiousCiv(m_pPlayer);
#endif
...
#ifdef AMB_AI_TREASURY_MANAGEMENT
void CvEconomicAI::DoTreasurySplit()
{
	//Look at the cash flow since previous turn
	int iCurrentTreasury = GetPlayer()->GetTreasury()->GetGold();
	int iSum = GetMoneyForRushs()+GetMoneyForPlots()+GetMoneyForMinorGifts()+GetMoneyForUnitUpgrades();
	int iChangeSplit =0;
	int iDelta = 0; 
	//If need to rebalance
	if(GetMoneyForRushs()==0||GetMoneyForPlots()==0||GetMoneyForMinorGifts()==0||GetMoneyForUnitUpgrades()==0)
	{
		iChangeSplit = iCurrentTreasury / 4;
		SetMoneyForRushs(iChangeSplit);
		SetMoneyForPlots(iChangeSplit);
		SetMoneyForMinorGifts(iChangeSplit);
		SetMoneyForUnitUpgrades(iChangeSplit);
	}
	else
	{
		iDelta = iCurrentTreasury - iSum;
		iChangeSplit = iDelta /4;

		ChangeMoneyForRushs(iChangeSplit);
		ChangeMoneyForPlots(iChangeSplit);
		ChangeMoneyForMinorGifts(iChangeSplit);
		ChangeMoneyForUnitUpgrades(iChangeSplit);
		//These 4 values are our maximum budget for each category
		//There is room for a more conditional and tailored split now !
	}

	//Does things balance now ?
	iSum = GetMoneyForRushs()+GetMoneyForPlots()+GetMoneyForMinorGifts()+GetMoneyForUnitUpgrades();
	iDelta = iCurrentTreasury - iSum;
	ChangeMoneyForRushs(iDelta);

	AMB_LOG("Total %i Sheet: %i %i %i %i",iCurrentTreasury,GetMoneyForRushs(),GetMoneyForPlots(),GetMoneyForMinorGifts(),GetMoneyForUnitUpgrades());
}
#endif
...
void CvEconomicAI::DoHurry()
{
	...
	#ifdef AMB_AI_TREASURY_MANAGEMENT
		int iMaximumGoldToHurry = GetMoneyForRushs();
		if (iMaximumGoldToHurry < 300)
		{
			return;
		}
		int iBestGoldCost = MAX_INT;
		int iGoldCost = MAX_INT;
	#else
	...
	#ifdef AMB_AI_TREASURY_MANAGEMENT
						iGoldCost = pLoopCity->GetPurchaseCost((UnitTypes)pOrder->iData1);
	#endif
						break;
	...
	#ifdef AMB_AI_TREASURY_MANAGEMENT
						iGoldCost = pLoopCity->GetPurchaseCost((BuildingTypes)pOrder->iData1);
	#endif
						break;
					case ORDER_CREATE:
	...
	#ifdef AMB_AI_TREASURY_MANAGEMENT
						iGoldCost = pLoopCity->GetPurchaseCost((ProjectTypes)pOrder->iData1);
	#endif
						break;
					case ORDER_PREPARE:
	...
	#ifdef AMB_AI_TREASURY_MANAGEMENT
				if (iGoldCost > iMaximumGoldToHurry)
				{
					continue;
				}
	#endif
	...
	if(pBestHurryCity != NULL)
	{
		...
		if(iBestGoldCost>0)
		{
			ChangeMoneyForRushs(-iBestGoldCost);
			AMB_LOG("Rush:%i",iBestGoldCost);
		}
	#endif
...
#ifdef AMB_AI_TREASURY_MANAGEMENT
	if(iCurrentCost < GetMoneyForPlots() && iGoldForHalfCost > iCurrentCost)
#else
...
#ifdef AMB_AI_TREASURY_MANAGEMENT
					ChangeMoneyForPlots(-iCost);
					AMB_LOG("Buy plot:%i",iCost);
#endif
...
bool EconomicAIHelpers::IsTestStrategy_EarlyExpansion(CvPlayer* pPlayer)
{
	...
	iDesiredCities = (iDesiredCities * iFlavorExpansion) / 5;
#else
	iDesiredCities = (iDesiredCities * iFlavorExpansion) / max(iFlavorGrowth, 1);
#endif

#ifdef AMB_DIFFICULTY_ID_ADJUSTMENT
	int iDifficulty = min(4,max(0,GC.getGame().getHandicapInfo().GetID() - 2));
#else
...
#ifdef AMB_AI_NUM_CITIES_TWEAKS
		//Always try to at least get 4 cities
		if (pPlayer->getNumCities() < 5)
		{
			++iNumExtraSettlers;
		}
		if (iFlavorExpansion > 7)
		{
			++iNumExtraSettlers;
		}
		if (pPlayer->getNumCities() < 6 && pPlayer->GetCurrentEra() >= 2)
		{
			++iNumExtraSettlers;
		}
		if (pPlayer->getNumCities() < 7 && pPlayer->GetCurrentEra() >= 3)
		{
			++iNumExtraSettlers;
		}
		if (pPlayer->getNumCities() < 8 && pPlayer->GetCurrentEra() >= 4)
		{
			++iNumExtraSettlers;
		}
#else
...
bool EconomicAIHelpers::IsTestStrategy_NeedHappiness(EconomicAIStrategyTypes eStrategy, CvPlayer* pPlayer)
{
	...
	if(pPlayer->getTotalPopulation() > 0 && pPlayer->GetUnhappiness() > 0)
	{
		...
		//Why base it on flavors ? Below 4 happiness is bad for everybody
		//Should aim for 1 happiness per city minimum
		int iCityNumberModifier = pPlayer->getNumCities() + pPlayer->GetNumUnitsWithUnitAI(UNITAI_SETTLE, true);
		if(iExcessHappiness < iCityNumberModifier)
		{
			return true;
		}
#else
...
bool EconomicAIHelpers::IsTestStrategy_EnoughArchaeologists(CvPlayer* pPlayer)
{
	...
	int iNumArchaeologists = pPlayer->GetNumUnitsWithUnitAI(UNITAI_ARCHAEOLOGIST, true);
#ifdef AMB_BETTER_POLICY_STRATEGY
	PolicyTypes eExpFinisher = (PolicyTypes) GC.getInfoTypeForString("POLICY_AESTHETICS_FINISHER", true /*bHideAssert*/);
#else
	PolicyTypes eExpFinisher = (PolicyTypes) GC.getInfoTypeForString("POLICY_EXPLORATION_FINISHER", true /*bHideAssert*/);
#endif
...
#ifdef AMB_BETTER_POLICY_STRATEGY
bool EconomicAIHelpers::IsTestStrategy_IsReligiousCiv(CvPlayer* pPlayer)
{
	if(pPlayer->isHuman())
	{
		return false;
	}
	FlavorTypes eFlavorReligion = (FlavorTypes)GC.getInfoTypeForString("FLAVOR_RELIGION", true);
	int iReligionFlavor = pPlayer->GetFlavorManager()->GetPersonalityIndividualFlavor(eFlavorReligion);

	if (iReligionFlavor > 6)
	{
		return true;
	}
	return false;
...
/*
bool EconomicAIHelpers::IsTestStrategy_HasMerchantConfederacy(CvPlayer* pPlayer)
{
	if(pPlayer->isHuman())
	{
		return false;
	}
	PolicyTypes ePolicy = (PolicyTypes)GC.getInfoTypeForString("POLICY_MERCHANT_CONFEDERACY", true);
	if (ePolicy != NO_POLICY)
	{
		if (pPlayer->GetPlayerPolicies()->HasPolicy(ePolicy))
		{
			return true;
		}
	}
