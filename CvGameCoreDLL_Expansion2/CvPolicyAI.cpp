...
#ifdef AMB_POLICY_BRANCH_ALL_POLICIES
	m_PolicyAIWeightsNoPropagates.clear();
#endif
...
#ifdef AMB_POLICY_BRANCH_ALL_POLICIES
				m_PolicyAIWeightsNoPropagates.push_back(i, 0);
#endif
			}
		}
	}
...
void CvPolicyAI::Read(FDataStream& kStream)
{
...
#ifdef AMB_POLICY_BRANCH_ALL_POLICIES
	m_PolicyAIWeightsNoPropagates.resize(uiPolicyArraySize);
#endif
...
#ifdef AMB_POLICY_BRANCH_ALL_POLICIES
		m_PolicyAIWeightsNoPropagates.SetWeight(uiIndex, 0);
#endif
	}
	...
	for(uint uiIndex = 0; uiIndex < uiPolicyCount; ++uiIndex)
	{
		...
		if(ePolicy != NO_POLICY && (uint)ePolicy < uiPolicyArraySize)
		{
			m_PolicyAIWeightsNoPropagates.SetWeight((uint)ePolicy, iWeight);
		}
	}	
#endif
...
void CvPolicyAI::Write(FDataStream& kStream)
{
	...
	{
		CvInfosSerializationHelper::WriteHashed(kStream, static_cast<const PolicyTypes>(i));
		kStream << m_PolicyAIWeightsNoPropagates.GetWeight(i);
	}
#endif
...
void CvPolicyAI::AddFlavorWeights(FlavorTypes eFlavor, int iWeight, int iPropagationPercent)
{
	...
	if (eFlavor == (FlavorTypes)GC.getInfoTypeForString("FLAVOR_HAPPINESS"))
	{
		...
		{
			iWeight *= AUI_POLICY_MULTIPLY_HAPPINESS_WEIGHT_WHEN_UNHAPPY;
		}
	...
	if (eFlavor == (FlavorTypes)GC.getInfoTypeForString("FLAVOR_RELIGION"))
	{
		...
		//MODIFIED BY ACKEN, removed the no pantheon = no piety and removed the bonus when you have a religion.
		if (!pPlayerReligions->HasCreatedReligion() && GC.getGame().GetGameReligions()->GetNumReligionsStillToFound() == 0)
		{
			iWeight /= AUI_POLICY_DIVIDE_RELIGION_WEIGHT_WHEN_NO_RELIGION;
		}
	}
#endif // AUI_POLICY_DIVIDE_RELIGION_WEIGHT_WHEN_NO_RELIGION
	...
	for(iPolicy = 0; iPolicy < pkPolicyEntries->GetNumPolicies(); iPolicy++)
	{
		...
		if(entry)
			paiTempWeights[iPolicy] = entry->GetFlavorValue(eFlavor) * iWeight; //iFlavorValue * Valueflavor of the policy
		else
			paiTempWeights[iPolicy] = 0;
#ifdef AMB_POLICY_BRANCH_ALL_POLICIES
	m_PolicyAIWeightsNoPropagates.IncreaseWeight(iPolicy, paiTempWeights[iPolicy]);
#endif
	}
...
#ifndef AMB_AI_POLICY_REMOVED_ECULTUREGS_CHECKER
			...
			if(pkPolicyBranchInfo)
			{
				...
				if(pPlayer->GetPlayerPolicies()->CanUnlockPolicyBranch(ePolicyBranch) && !pPlayer->GetPlayerPolicies()->IsPolicyBranchUnlocked(ePolicyBranch))
				{
					...
					if(IsBranchEffectiveInGame(ePolicyBranch))
					{
						...
						iBranchWeight += WeighBranch(ePolicyBranch);//NoteAcken this function add all weights on policies with no prereq + opener


																	//This causes trees with less policies without prereq to be undervalued !!!!
																	//But it also rewards requisite at 25% per link
																	//Shouldn't the branch be weighted regardless of prerequisites ? so that the whole tree is valued rather than the opener alone
																	//Weights are : SUM_Policies of (Flavor_personality + Grand strategy_mod)*PolicyFlavor + propagation
#ifdef AMB_AI_POLICY_REMOVED_ECULTUREGS_CHECKER
//AMB_LOG("WeighBranch is %i for %i", iBranchWeight, ePolicyBranch);
						iBranchWeight *= (100 - m_iPolicyWeightPercentDropNewBranch); //was too much with new method
						iBranchWeight /= 100;
						if(iBranchLoop != 8)
						{
							iBranchWeight /= 5; //averaged But apparently the weights are way too similar now
						}
//AMB_LOG("WeighBranch is %i for %i", iBranchWeight, ePolicyBranch);
#else
						...
						if(eCurrentGrandStrategy == eCultureGrandStrategy) //NoteAcken Why ?
						{
							iBranchWeight /= 3;
						}
...
void CvPolicyAI::DoChooseIdeology(CvPlayer *pPlayer)
{
...
#ifdef AMB_GS_IDEOLOGY_CHOOSE_BY_FLAVORS
	//NoteAcken This code is really stupid since it doesn't use any of the flavors of the tress !!
	//The priority should be multiplied with how much each tree favors a victory or another. Otherwise you may end up with a GC culture
	//going with autocracy just because their priority for diplo is low ! Nonsense.

	//I think it should use the weigths modified for advant rather than the priorities.
	...
	int iDiploPriority =0;
	int iTechPriority =0;
	for(iGrandStrategiesLoop = 0; iGrandStrategiesLoop < pPlayer->GetGrandStrategyAI()->GetAIGrandStrategies()->GetNumAIGrandStrategies(); iGrandStrategiesLoop++)
	{
		eGrandStrategy = (AIGrandStrategyTypes) iGrandStrategiesLoop;
		pGrandStrategy = pPlayer->GetGrandStrategyAI()->GetAIGrandStrategies()->GetEntry(iGrandStrategiesLoop);
		strGrandStrategyName = (CvString) pGrandStrategy->GetType();
		if(strGrandStrategyName == "AIGRANDSTRATEGY_CONQUEST")
		{
			iConquestPriority = max(0, pPlayer->GetGrandStrategyAI()->GetGrandStrategyPriority(eGrandStrategy));
		}
		else if(strGrandStrategyName == "AIGRANDSTRATEGY_CULTURE")
		{
			iCulturePriority = max(0, pPlayer->GetGrandStrategyAI()->GetGrandStrategyPriority(eGrandStrategy));
		}
		else if(strGrandStrategyName == "AIGRANDSTRATEGY_UNITED_NATIONS")
		{
			iDiploPriority = max(0, pPlayer->GetGrandStrategyAI()->GetGrandStrategyPriority(eGrandStrategy));
		}
		else if(strGrandStrategyName == "AIGRANDSTRATEGY_SPACESHIP")
		{
			iTechPriority = max(0, pPlayer->GetGrandStrategyAI()->GetGrandStrategyPriority(eGrandStrategy));
		}
	}

	int iAutocracyWeight = 0;
	int iFreedomWeight = 0;
	int iOrderWeight = 0;
	int iPolicyLoop;
	for(iPolicyLoop = 0; iPolicyLoop < m_pCurrentPolicies->GetPolicies()->GetNumPolicies(); iPolicyLoop++)
	{
		if(m_pCurrentPolicies->GetPolicies()->GetPolicyEntry(iPolicyLoop)->GetLevel() > 0)
		{
			int iWeight = 0;
			iWeight += m_PolicyAIWeights.GetWeight(iPolicyLoop);
			if(m_pCurrentPolicies->GetPolicies()->GetPolicyEntry(iPolicyLoop)->GetPolicyBranchType()==eFreedomBranch)
				iFreedomWeight += iWeight;
			if(m_pCurrentPolicies->GetPolicies()->GetPolicyEntry(iPolicyLoop)->GetPolicyBranchType()==eAutocracyBranch)
				iAutocracyWeight += iWeight;
			if(m_pCurrentPolicies->GetPolicies()->GetPolicyEntry(iPolicyLoop)->GetPolicyBranchType()==eOrderBranch)
				iOrderWeight += iWeight;
		}
	}
#else
...
#ifdef AMB_GS_IDEOLOGY_CHOOSE_BY_FLAVORS
	iFreedomTotal *= iFreedomWeight;
	iAutocracyTotal *= iAutocracyWeight;
	iOrderTotal *= iOrderWeight;
#endif
...
{
	if(iPropagationLevel < m_iPolicyWeightPropagationLevels) //2 levels default
	{
		int iPropagatedWeight = iWeight * iPropagationPercent / 100; //25% default
				...
				if(!m_pCurrentPolicies->HasPolicy(ePolicyLoop))
				{
					...
					if(pkLoopPolicyInfo->GetPolicyBranchType() == eBranch)
					{
...
#ifdef AMB_POLICY_BRANCH_ALL_POLICIES
							iWeight += m_PolicyAIWeightsNoPropagates.GetWeight(iPolicyLoop);
						...
						if(pkLoopPolicyInfo->GetPrereqAndPolicies(0) == NO_POLICY)
						{
							...
							iWeight += m_PolicyAIWeights.GetWeight(iPolicyLoop); // Weights are the policy + the propagation of their underlying (up to 2 links) but at a 25% discout
...
#endif
//#ifdef AMB_MOD_LOG
							//AMB_LOG("Step iWeight %i for %i in %i",iWeight,pkLoopPolicyInfo->GetPolicyBranchType());
//#endif					
					}
				}
			}
...
#ifdef AMB_POLICY_BRANCH_ALL_POLICIES
		iWeight += m_PolicyAIWeightsNoPropagates.GetWeight(pkPolicyBranchInfo->GetFreePolicy());
#else
