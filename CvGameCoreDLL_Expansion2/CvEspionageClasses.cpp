...
#ifdef AMB_BETTER_SPY_CATCHUP
				iSpyResult -= pSpy->m_eRank * 30;
				int iCityOwnerTechs = GET_TEAM(GET_PLAYER(eCityOwner).getTeam()).GetTeamTechs()->GetNumTechsKnown();
				int iSpyOwnerTechs = GET_TEAM(GET_PLAYER(ePlayer).getTeam()).GetTeamTechs()->GetNumTechsKnown();
				iSpyResult -= max(0,iCityOwnerTechs-iSpyOwnerTechs)*5;
#endif
	...
	case SPY_STATE_GATHERING_INTEL:
	{
		if(pCity)
		{
			...
			if(iSpyIndex >= 0)
			{
				...
				iSpyRank += m_pPlayer->GetCulture()->GetInfluenceMajorCivSpyRankBonus(eCityOwner);
#ifdef AMB_BETTER_SPY
				iResult = (iBaseYieldRate * (100 + (GC.getESPIONAGE_GATHERING_INTEL_RATE_BY_SPY_RANK_PERCENT() * iSpyRank) 
					+ iCityEspionageModifier + iPlayerEspionageModifier + iTheirPoliciesEspionageModifier 
					+ iMyPoliciesEspionageModifier))/100; 
				double ibonusEra=(pow((double) max(0,GET_PLAYER(eCityOwner).GetCurrentEra()-2)*0.45,1.50)+1.0)*100.0; /*same rate during renaissance, better rate later*/
				iResult *= (int) ibonusEra;
				iResult /= 100;
...
#ifdef AMB_BETTER_SPY
			int iCityOwnerTechs = GET_TEAM(GET_PLAYER(ePlayer).getTeam()).GetTeamTechs()->GetNumTechsKnown();
			int iSpyOwnerTechs = GET_TEAM(m_pPlayer->getTeam()).GetTeamTechs()->GetNumTechsKnown();
			uiMaxTechCostAdjusted *= -min(max(0,iCityOwnerTechs-iSpyOwnerTechs)*2,20) + GC.getESPIONAGE_GATHERING_INTEL_COST_PERCENT();
...
int CvPlayerEspionage::GetCoupChanceOfSuccess(uint uiSpyIndex)
{
	...
	iResultPercentage += m_pPlayer->GetPlayerPolicies()->GetNumericModifier(POLICYMOD_EXTRACOUPCHANCE);
	int iModifier = m_pPlayer->GetPlayerPolicies()->GetNumericModifier(POLICYMOD_REMOVECOUPCAPCHANCE);
	if(iResultPercentage > 85 + iModifier)
	{
		iResultPercentage = 85 + iModifier;
	}
	if(iResultPercentage > 100)
	{
