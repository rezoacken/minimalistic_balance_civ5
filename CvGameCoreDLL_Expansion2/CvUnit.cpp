...
#ifdef AMB_FIXED_GS_BULB
	, m_iBulbValue(0) //BulbValue for fixed bulbs
#endif
#ifdef AMB_FIXED_GW_BULB
	, m_iTreatyValue(0)
#endif
...
void CvUnit::initWithNameOffset(int iID, UnitTypes eUnit, int iNameOffset, UnitAITypes eUnitAI, PlayerTypes eOwner, int iX, int iY, DirectionTypes eFacingDirection, bool bNoMove, bool bSetupGraphical, int iMapLayer, int iNumGoodyHutsPopped)
{
	...
	{
		m_iTreatyValue = getGivePoliciesCulture();
	}
#endif

#ifdef AMB_FIXED_GS_BULB
	if (getUnitInfo().GetBaseBeakersTurnsToCount()>0) //If unit can discover
	{
		m_iBulbValue = getDiscoverAmount(); //modAcken fixes the bulb value
	}
#endif
...
void CvUnit::kill(bool bDelay, PlayerTypes ePlayer /*= NO_PLAYER*/)
{
	...
	if(ePlayer != NO_PLAYER)
	{
		if(!isBarbarian() && !GET_PLAYER(ePlayer).isBarbarian())
		{
			...
			//If a unit was killed let's put the no activity timer to 0
			GET_PLAYER(getOwner()).GetDiplomacyAI()->ChangeWarNoUnitTimer(ePlayer,-5);
			GET_PLAYER(ePlayer).GetDiplomacyAI()->SetWarNoUnitTimer(getOwner(),0);
#endif
		}
...
#ifdef AMB_FIXED_GW_BULB
int CvUnit::GetTreatyValue() const
{
	return m_iTreatyValue;
}
#endif

#ifdef AMB_FIXED_GS_BULB
int CvUnit::GetBulbValue() const //modAcken Returns the value of the bulb
{
	return m_iBulbValue;
}
#endif
...
bool CvUnit::discover()
{
	...
	// Beakers boost based on previous turns
#if defined(AMB_FIXED_GS_BULB)
	int iBeakersBonus = m_iBulbValue; //modAcken No longer changes from turn to turn. Now gives the fixed value at creation
#else
...
#ifdef AMB_DEFEND_CS_INFLUENCE
void CvUnit::DoTestWarmongerThreatToMinorsWithThisUnitsDeath(PlayerTypes eKillingPlayer)
{
	VALIDATE_OBJECT
	...
	for(int iMinorLoop = MAX_MAJOR_CIVS; iMinorLoop < MAX_CIV_PLAYERS; iMinorLoop++)
	{
		...
		if(GET_PLAYER(eMinor).isAlive())
		{
			//Minor must be at war with unit owner
			if(!GET_PLAYER(eMinor).GetMinorCivAI()->IsAtWarWithPlayersTeam(getOwner()))
			{
				continue;
			}
			...
			{
				GET_PLAYER(eMinor).GetMinorCivAI()->DoThreateningWarmongerKilled(eKillingPlayer, getX(), getY());
				continue;
			}
...
int CvUnit::GetRangeCombatDamage(const CvUnit* pDefender, CvCity* pCity, bool bIncludeRand, int iAssumeExtraDamage) const
{
...
#ifdef AMB_RANGED_BOOST_GARRISONED
	if(IsGarrisoned())
	{
		//AMB_LOG("Range attack Was %i", iAttackerStrength);
		iAttackerStrength*=100+AMB_RANGED_BOOST_GARRISONED;
		iAttackerStrength/=100;
		//AMB_LOG("Range attack is %i", iAttackerStrength);
	}
#endif
	...
	{
		iDefenderStrength = pCity->getStrengthValue();
#ifdef AMB_CITY_COVER2
		if(GET_PLAYER(getOwner()).isHuman())
		{
			//AMB_LOG("City Defense %i", iDefenderStrength);
			iDefenderStrength *= (100+AMB_CITY_COVER2);
			iDefenderStrength /= 100;
			//AMB_LOG("City Defense is %i", iDefenderStrength);
		}
#endif
	}
...
int CvUnit::GetReverseGreatGeneralModifier()const
{
	...
	{
#ifdef AUI_FAST_COMP
		iMaxDX = iGreatGeneralRange - FASTMAX(0, iY);
		for (iX = -iGreatGeneralRange - FASTMIN(0, iY); iX <= iMaxDX; iX++) // MIN() and MAX() stuff is to reduce loops (hexspace!)
#else
...
void CvUnit::read(FDataStream& kStream)
{
	...
	kStream >> *m_pReligion;
#ifdef AMB_FIXED_GW_BULB
	kStream >> m_iTreatyValue;
#endif
#ifdef AMB_FIXED_GS_BULB
	kStream >> m_iBulbValue;
#endif
