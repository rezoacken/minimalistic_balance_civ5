...
CvPlayer::CvPlayer() :
	...
	, m_iReducedResistance("CvPlayer::m_iReducedResistance", m_syncArchive)
#endif
	...
	, m_iGreatScientistCreated(0)
	, m_iGreatEngineerCreated(0)
	, m_iGreatMerchantCreated(0)
#endif
...
#ifdef AMB_CIV_CHANGES
	if(iFreeCount > 0 && GetPlayerTraits()->GetExtraFoundedCityTerritoryClaimRange() > 0)
	{
...
void CvPlayer::acquireCity(CvCity* pOldCity, bool bConquest, bool bGift)
{
...
#ifdef AMB_AI_NO_ACTIVITY_WHITE_PEACE
			//If a city was captured let's put the no activity timer to 0
			GetDiplomacyAI()->SetWarNoCityTimer(pOldCity->getOwner(),0);
			GET_PLAYER(pOldCity->getOwner()).GetDiplomacyAI()->SetWarNoCityTimer(GetID(),0);
#endif
	...
	if (bConquest)
	{
		if(!isMinorCiv())
		{
			...
			if (bDoWarmonger)
			{
#ifdef AMB_AI_WARMONGER
				if(GetDiplomacyAI()->HasCasusBelliAgainst(pOldCity->getOwner()))
				{
					AMB_LOG("Removed Casus Belli for %s on %s",getName(),GET_PLAYER(pOldCity->getOwner()).getName());
					GetDiplomacyAI()->SetCasusBelliAgainst(pOldCity->getOwner(),false);
					GetDiplomacyAI()->SetCasusBelliTimer(pOldCity->getOwner(),0);
				}
				else
				{
...
#ifdef AMB_BUILDING_REDUCED_RESISTANCE
			int iReductionMod = GetReducedResistance();
			iResistanceTurns = iResistanceTurns * (100+iReductionMod)/100;
#endif
...
#ifdef AMB_AI_WARMONGER
			pDiploAI->SetCasusBelliAgainst(eMePlayer,false);
			pDiploAI->SetCasusBelliTimer(eMePlayer,0);
#endif
...
#ifdef AMB_POLICY_FREEDEFENSE
	int iNumFreeDefenseBuildings = GetNumCitiesFreeDefenseBuilding();
	if(iNumFreeDefenseBuildings > 0)
	{
		BuildingTypes eBuilding = pCity->ChooseFreeDefenseBuilding();
		if(eBuilding != NO_BUILDING)
		{
			pCity->GetCityBuildings()->SetNumFreeBuilding(eBuilding, 1);
		}
		...
		{
			pCity->SetOwedDefenseBuilding(true);
		}

		ChangeNumCitiesFreeDefenseBuilding(-1);
	}
#endif
...
#ifdef AMB_CIV_CHANGES
		iTempMod = m_pTraits->GetBuildingProductionModifier();
		iMultiplier += iTempMod;
...
{
#if defined(AMB_MOD_H)
		int iCulture=0;
		iCulture = GetPlayerTraits()->GetYieldChangePerTradePartner(YIELD_CULTURE) * GetTrade()->GetNumDifferentTradingPartners();
	#if defined(AMB_POLICY_YIELDCHANGEPERMINORTRADEROUTE)
		iCulture += GetPlayerPolicies()->GetYieldChangePerMinorTradeRoute(YIELD_CULTURE) * GetTrade()->GetNumberOfCityStateTradeRoutes();
	#endif
	#if defined(AMB_POLICY_YIELDCHANGEPERSEAINTERNATIONALTRADEROUTE)
		iCulture += GetPlayerPolicies()->GetYieldChangePerSeaInternationalTradeRoute(YIELD_CULTURE) * GetTrade()->GetNumTradeInternationalRoutes(true);
	#endif
	return iCulture;
#endif
#ifndef AMB_POLICY_YIELDCHANGEPERMINORTRADEROUTE
	#ifndef AMB_POLICY_YIELDCHANGEPERSEAINTERNATIONALTRADEROUTE
...
int CvPlayer::GetCultureYieldFromPreviousTurns(int iGameTurn, int iNumPreviousTurnsToCount)
{
	...
	for (int iI = 0; iI < iNumPreviousTurnsToCount; iI++)
	{
		...
		{
#ifdef AMB_FIX_ONE_TURN
			iSum += GetTotalJONSCulturePerTurn();
#else
...
#ifdef AMB_POLICY_FREEDEFENSE
//	--------------------------------------------------------------------------------
/// Cities remaining to get a free defense building
int CvPlayer::GetNumCitiesFreeDefenseBuilding() const
{
	return m_iNumCitiesFreeDefenseBuilding;
}
//	--------------------------------------------------------------------------------
/// Changes number of cities remaining to get a free defense building
void CvPlayer::ChangeNumCitiesFreeDefenseBuilding(int iChange)
{
	if(iChange != 0)
		m_iNumCitiesFreeDefenseBuilding += iChange;
}
#endif
//	--------------------------------------------------------------------------------
...
int CvPlayer::GetHappinessFromPolicies() const
{
	...
	//modAcken add extra code for extra happiness from following cities
#if defined(AMB_POLICY_EXTRAHAPPINESSPERFOLLOWINGCITY)
	int iExtraHappinessForFollowingCity = m_pPlayerPolicies->GetNumericModifier(POLICYMOD_EXTRA_HAPPINESS_FOR_FOLLOWING_CITY);
	if(m_pReligions->GetReligionCreatedByPlayer() > RELIGION_PANTHEON && iExtraHappinessForFollowingCity>0)
	{
		ReligionTypes eReligion = GetReligions()->GetReligionCreatedByPlayer();
		int iTotalCities = GC.getGame().GetGameReligions()->GetNumCitiesFollowing(eReligion);
		int iForeignCities = GetReligions()->GetNumForeignCitiesFollowing();
		iHappiness += (iTotalCities - iForeignCities);
		iHappiness +=  iForeignCities / 3;
	}
#endif
	//end mod
...
int CvPlayer::GetHappinessFromReligion()
{
	...
	if(eFoundedReligion != NO_RELIGION)
	{
		...
		if(pReligion)
		{
			...
			{
#ifdef AMB_BELIEF_PEACELOVING_NOTATPEACE
...
#ifdef AMB_BUILDING_REDUCED_RESISTANCE
int CvPlayer::GetReducedResistance() const
{
	return m_iReducedResistance;
}
void CvPlayer::ChangeReducedResistance(int iChange)
{
	if(iChange != 0)
	{
		m_iReducedResistance += iChange;
	}
}
#endif
//	--------------------------------------------------------------------------------
...
#ifdef AMB_PARTIAL_GP_COUNTERS
int CvPlayer::getGreatScientistCreated() const
{
	return m_iGreatScientistCreated;
}
int CvPlayer::getGreatEngineerCreated() const
{
	return m_iGreatEngineerCreated;
}
int CvPlayer::getGreatMerchantCreated() const
{
	return m_iGreatMerchantCreated;
}
void CvPlayer::incrementGreatScientistCreated()
{
	m_iGreatScientistCreated++;
}
void CvPlayer::incrementGreatEngineerCreated()
{
	m_iGreatEngineerCreated++;
}
void CvPlayer::incrementGreatMerchantCreated()
{
	m_iGreatMerchantCreated++;
}
#endif
...
int CvPlayer::GetScienceFromOtherPlayersTimes100() const
{
	...
	#if defined(AMB_POLICY_YIELDCHANGEPERMINORTRADEROUTE)
		iScience += GetPlayerPolicies()->GetYieldChangePerMinorTradeRoute(YIELD_SCIENCE)*GetTrade()->GetNumberOfCityStateTradeRoutes()*100;
	#endif
...
void CvPlayer::processPolicies(PolicyTypes ePolicy, int iChange)
{
	...
	if(pPolicy->GetInstantSpyRankChange()>0)
	{
		CvPlayerEspionage* pEspionage = GetEspionage();
		CvAssertMsg(pEspionage, "pEspionage is null! What's up with that?!");
		if(pEspionage)
	...
	if(pPolicy->GetSpyRankChange()>0)
	{
		ChangeStartingSpyRank(pPolicy->GetSpyRankChange());
	}
#endif
...
#ifdef AMB_POLICY_FREEDEFENSE
	int iNumCitiesFreeDefenseBuilding = pPolicy->GetNumCitiesFreeDefenseBuilding();
#endif
...
#ifdef AMB_POLICY_FREEDEFENSE
		if(iNumCitiesFreeDefenseBuilding > 0)
		{
			BuildingTypes eDefenseBuilding = pLoopCity->ChooseFreeDefenseBuilding();
			if(eDefenseBuilding != NO_BUILDING)
			{
				pLoopCity->GetCityBuildings()->SetNumFreeBuilding(eDefenseBuilding, 1);

				if(pLoopCity->getFirstBuildingOrder(eDefenseBuilding) == 0)
				{
					pLoopCity->clearOrderQueue();
					pLoopCity->chooseProduction();		// Send a notification to the user that what they were building was given to them, and they need to produce something else.
				}
			...
			{
				pLoopCity->SetOwedDefenseBuilding(true);
			}

			// Decrement cities left to get free culture building (at end of loop we'll set the remainder)
			iNumCitiesFreeDefenseBuilding--;
		}
#endif
...
#ifdef AMB_POLICY_IDEOLOGIES
		pLoopCity->ChangeExtraHitPoints(pPolicy->GetExtraCityHitPoints());
		if(pPolicy->GetExtraCityHitPoints()>0)
		{
...
#ifdef AMB_POLICY_FREEDEFENSE
	ChangeNumCitiesFreeDefenseBuilding(iNumCitiesFreeDefenseBuilding);
#endif
							...
							for(int iUnitLoop = 0; iUnitLoop < iNumFreeUnits; iUnitLoop++)
							{
								...
								if (pNewUnit)
								{
									...
									else if(pNewUnit->getUnitInfo().IsFoundReligion())
									{
										...
										if(eReligion > RELIGION_PANTHEON && GetPlayerPolicies()->GetTotalProphetSpawnProductionBonusPerFollower()>0)
										{
											//number of followers total for our religion
											int iBonus = GC.getGame().GetGameReligions()->GetNumFollowers(GetReligions()->GetReligionCreatedByPlayer());
											iBonus *= GetPlayerPolicies()->GetTotalProphetSpawnProductionBonusPerFollower();
											iBonus = min(iBonus,100);
											iBonus *= GC.getGame().getGameSpeedInfo().getConstructPercent()/100;
											//Increase city production
											if(getCapitalCity()->isProduction())
											{
												getCapitalCity()->changeProduction(iBonus);
											}
											//unless nothing is in production
											else
											{
												getCapitalCity()->setOverflowProduction(getCapitalCity()->getOverflowProduction()+iBonus);
											}
											if (getCapitalCity()->plot() && getCapitalCity()->plot()->GetActiveFogOfWarMode() == FOGOFWARMODE_OFF)
											{
												char text[256] = {0};
												sprintf_s(text, "[COLOR_YELLOW]+%d[ENDCOLOR][ICON_PRODUCTION]", iBonus);
												float fDelay = GC.getPOST_COMBAT_TEXT_DELAY() * 1.0f;
												DLLUI->AddPopupText(getCapitalCity()->getX(), getCapitalCity()->getY(), text, fDelay);
											}
										}
										if(eReligion > RELIGION_PANTHEON && GetPlayerPolicies()->GetTotalProphetSpawnGoldBonusPerFollower()>0)
										{
											//number of followers total for our religion
											int iBonus = GC.getGame().GetGameReligions()->GetNumFollowers(GetReligions()->GetReligionCreatedByPlayer());
											iBonus *= GetPlayerPolicies()->GetTotalProphetSpawnProductionBonusPerFollower();
											...
											iBonus *= GC.getGame().getGameSpeedInfo().getConstructPercent()/100;
											//Increase gold 
											GetTreasury()->SetGold(GetTreasury()->GetGold()+iBonus);
											if (getCapitalCity()->plot() && getCapitalCity()->plot()->GetActiveFogOfWarMode() == FOGOFWARMODE_OFF)
											{
												char text[256] = {0};
										...
										if(eReligion > RELIGION_PANTHEON && GetPlayerPolicies()->GetTotalProphetSpawnGoldenAgeBonusPerFollower()>0)
										{
											//number of followers total for our religion
											int iBonus = GC.getGame().GetGameReligions()->GetNumFollowers(GetReligions()->GetReligionCreatedByPlayer());
											iBonus *= GetPlayerPolicies()->GetTotalProphetSpawnProductionBonusPerFollower();
											...
											ChangeGoldenAgeProgressMeter(iBonus);
											if (getCapitalCity()->plot() && getCapitalCity()->plot()->GetActiveFogOfWarMode() == FOGOFWARMODE_OFF)
											{
												char text[256] = {0};
...
#ifdef AMB_PARTIAL_GP_COUNTERS
	kStream >> m_iGreatScientistCreated;
	kStream >> m_iGreatEngineerCreated;
	kStream >> m_iGreatMerchantCreated;
#endif
...
#ifdef AMB_POLICY_FREEDEFENSE
	kStream >> m_iNumCitiesFreeDefenseBuilding;
#endif 
...
int CvPlayer::getGrowthThreshold(int iPopulation) const
{
	...
	iThreshold /= 100;
#ifndef AMB_AIGROWTH_BONUS_CHANGE
	if(!isHuman() && !IsAITeammateOfHuman() && !isBarbarian())
	{
...
CvPlot* CvPlayer::GetBestSettlePlot(CvUnit* pUnit, bool bEscorted, int iArea) const
{
	...
	int iFlags = 0;
	if (!AUI_PLAYER_GET_BEST_SETTLE_PLOT_USE_PATHFINDER_FOR_EVALDISTANCE && bEscorted)
	...
	for(int iPlotLoop = 0; iPlotLoop < iNumPlots; iPlotLoop++)
	{
		...
		if(bEscorted || (!pPlot->isVisibleEnemyUnit(eOwner)))
		{
...
#endif // AUI_PLAYER_GET_BEST_SETTLE_PLOT_USE_MINIMUM_FERTILITY
			{
				...
				{
					pNode = NULL;
					iCurDistance = MAX_INT;

					if (AUI_PLAYER_GET_BEST_SETTLE_PLOT_USE_PATHFINDER_FOR_EVALDISTANCE &&
						GC.getIgnoreUnitsPathFinder().GeneratePath(pLoopCity->getX(), pLoopCity->getY(), pPlot->getX(), pPlot->getY(), iFlags, true))
					{
						pNode = GC.getIgnoreUnitsPathFinder().GetLastNode();
					}
					else if (!AUI_PLAYER_GET_BEST_SETTLE_PLOT_USE_PATHFINDER_FOR_EVALDISTANCE &&
						GC.GetTacticalAnalysisMapFinder().GeneratePath(pLoopCity->getX(), pLoopCity->getY(), pPlot->getX(), pPlot->getY(), iFlags, false))
					{
						pNode = GC.GetTacticalAnalysisMapFinder().GetLastNode();
					}
					if (pNode)
					{
						iCurDistance = pNode->m_iData2;
					}
