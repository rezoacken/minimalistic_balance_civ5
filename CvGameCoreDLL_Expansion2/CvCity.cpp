...
#ifdef AMB_WLTKD_RESET
	, m_iWLTKDNoResource("CvCity::m_iWLTKDNoResource", m_syncArchive)
#endif
...
#ifdef AMB_POLICY_FREEDEFENSE
	, m_bOwedDefenseBuilding(false)
#endif
{
	OBJECT_ALLOCATED
...
#ifdef AMB_AI_PRODUCTION_TWEAKS
//New function that return the culture from wonders
int CvCity::GetCultureFromWonders()
{
	int iCulture = 0;
	//Loop buildings
	for(int iBuildingLoop=0; iBuildingLoop < GC.getNumBuildingInfos();iBuildingLoop++)
	{
		...
		if(pkBuildingInfo)
		{
			...
			if(GetCityBuildings()->GetNumBuilding(eBuilding) > 0)
			{
				iCulture += pkBuildingInfo->GetYieldChange(YIELD_CULTURE);
			}
		}
	}
	return iCulture;
...
void CvCity::DoTestResourceDemanded()
{
	...
	else
	{
		if(eResource != NO_RESOURCE)
		{
			...
			{
				ChangeWLTKDNoResource(1);
				if(GetWLTKDNoResource()==GC.getCITY_RESOURCE_WLTKD_TURNS())
				{
					DoPickResourceDemanded();
					SetWLTKDNoResource(0);
				}
	...
	if(bIncludeBeliefDiscounts && (pkUnitInfo->IsSpreadReligion() || pkUnitInfo->IsRemoveHeresy()) && !pkUnitInfo->IsFoundReligion())
	{
		...
		if(eMajority > RELIGION_PANTHEON)
		{
			...
			if(pReligion)
			{
				...
				//NoteAcken: Follower and pantheon beliefs aren't supposed to change that
...
int CvCity::getProductionModifier(UnitTypes eUnit, CvString* toolTipSink) const
{
...
#ifdef AMB_BELIEF_SETTLERPRODUCTION
	if(pkUnitInfo->IsFound())
	{
		...
		{
			iReligionBonus += GC.getGame().GetGameReligions()->GetReligion(eMajority,getOwner())->m_Beliefs.GetSettlerProductionModifier();
			//Here I'm assuming there is only one such bonus ?
			if(iReligionBonus==0)
			{
				BeliefTypes eSecondaryPantheon = GetCityReligions()->GetSecondaryReligionPantheonBelief();
				if (eSecondaryPantheon != NO_BELIEF)
				{
					iReligionBonus += GC.GetGameBeliefs()->GetEntry(eSecondaryPantheon)->GetSettlerProductionModifier();
				}
			}
		}
		iTempMod = iReligionBonus;
		iMultiplier += iTempMod;
		if(iReligionBonus!=0)
		{
			GC.getGame().BuildProdModHelpText(toolTipSink, "TXT_KEY_PRODMOD_RELIGION_SETTLER", iTempMod);
		}
	}
#endif
				...
				if(eTech != NO_TECH)
				{
					...
					if(pEntry)
					{
						...
						if(eEra != NO_ERA)
						{
							...
							BeliefTypes eSecondaryPantheon = GetCityReligions()->GetSecondaryReligionPantheonBelief();
							//NoteAcken: No follower beleif increase Wonder production
							if (eSecondaryPantheon != NO_BELIEF)
							{
			...
			for(int iUnitLoop = 0; iUnitLoop < GC.getNumUnitInfos(); iUnitLoop++)
			{
				...
				if(pkUnitInfo)
				{
					for(iFreeUnitLoop = 0; iFreeUnitLoop < pBuildingInfo->GetNumFreeUnits(iUnitLoop); iFreeUnitLoop++)
					{
						...
						{
#ifdef AMB_TRUE_FREE_GP
							GetCityCitizens()->DoSpawnGreatPerson(eFreeUnitType, false /*bIncrementCount*/, true);
#else
...
#ifdef AMB_BUILDING_REDUCED_RESISTANCE
		if(pBuildingInfo->GetReducedResistance() != 0)
		{
			owningPlayer.ChangeReducedResistance(pBuildingInfo->GetReducedResistance() * iChange);
		}
#endif
	...
	for(int iYield = 0; iYield < NUM_YIELD_TYPES; iYield++)
	{
		...
		if(eNewMajority != NO_RELIGION)
		{
			...
			if(pReligion)
			{
				...
				//modAcken Changes for follower beleifs of the second religion
#if defined(AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS)
				BeliefTypes eSecondaryFollowerOne = GetCityReligions()->GetSecondaryReligionFollowerBelief(false);
				BeliefTypes eSecondaryFollowerTwo = GetCityReligions()->GetSecondaryReligionFollowerBelief(true);
#endif
#if defined(AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_YIELDS)
				if (eSecondaryFollowerOne != NO_BELIEF && getPopulation() >= GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->GetMinPopulation())
				{
					iReligionYieldChange += GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->GetCityYieldChange((YieldTypes)iYield);
					if (eSecondaryFollowerTwo != NO_BELIEF && getPopulation() >= GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->GetMinPopulation())
					{
						iReligionYieldChange += GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->GetCityYieldChange((YieldTypes)iYield);
					}
				}
#endif
				//endmod
				...
				if(IsRouteToCapitalConnected())
				{
					...
					//BeliefTypes eSecondaryPantheon = GetCityReligions()->GetSecondaryReligionPantheonBelief();
					//NoteAcken No follower Pantheon is supposed to change trade route yield
					if (eSecondaryPantheon != NO_BELIEF)
					{
				...
				if (GetCityCitizens()->GetTotalSpecialistCount() > 0)
				{
					...
					//modAcken Changes for follower beleifs of the second religion
					int iChangeAnySpecialist=pReligion->m_Beliefs.GetYieldChangeAnySpecialist((YieldTypes)iYield);
#if defined(AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_SPECIALIST)
					if (eSecondaryFollowerOne != NO_BELIEF)
					{
						iChangeAnySpecialist+=GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->GetYieldChangeAnySpecialist((YieldTypes)iYield);
						if (eSecondaryFollowerTwo != NO_BELIEF)
						{
							iChangeAnySpecialist+=GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->GetYieldChangeAnySpecialist((YieldTypes)iYield);
						}
					}
#endif
					switch(iYield)
					...
					ChangeJONSCulturePerTurnFromReligion(iChangeAnySpecialist);

					break;
					case YIELD_FAITH:

						ChangeFaithPerTurnFromReligion(iChangeAnySpecialist);

						break;
					default:
						ChangeBaseYieldRateFromReligion((YieldTypes)iYield, iChangeAnySpecialist);
						break;
					}
					//endmod
				}
				...
				for(int jJ = 0; jJ < GC.getNumBuildingClassInfos(); jJ++)
				{
					...
					if(eBuilding != NO_BUILDING)
					{
						if(GetCityBuildings()->GetNumBuilding(eBuilding) > 0)
						{
							...
							//modAcken Changes for follower beleifs of the second religion
#if defined(AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_BUILDINGS)
							if (eSecondaryFollowerOne != NO_BELIEF && getPopulation() >= GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->GetMinPopulation())
							{
								iYieldFromBuilding+=GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->GetBuildingClassYieldChange(eBuildingClass, (YieldTypes)iYield);
								if (eSecondaryFollowerTwo != NO_BELIEF && getPopulation() >= GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->GetMinPopulation())
								{
									iYieldFromBuilding+=GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->GetBuildingClassYieldChange(eBuildingClass, (YieldTypes)iYield);
								}
							}
#endif
							//endmod
...
#if defined(AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_WONDERS)
								//modAcken Changes for follower beleifs of the second religion
								if (eSecondaryFollowerOne != NO_BELIEF)
								{
									iYieldFromBuilding+=GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->GetYieldChangeWorldWonder((YieldTypes)iYield);
									if (eSecondaryFollowerTwo != NO_BELIEF)
									{
										iYieldFromBuilding+=GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->GetYieldChangeWorldWonder((YieldTypes)iYield);
									}
								}
#endif
								//endmod
...
#ifdef AMB_AIGROWTH_BONUS_CHANGE
	if(!GET_PLAYER(getOwner()).isHuman() && !GET_PLAYER(getOwner()).IsAITeammateOfHuman() && !GET_PLAYER(getOwner()).isBarbarian())
	{
		iNum *= GC.getGame().getHandicapInfo().getAIGrowthPercent();
		iNum /= 100;

		iNum *= std::max(0, ((GC.getGame().getHandicapInfo().getAIPerEraModifier() * GET_PLAYER(getOwner()).GetCurrentEra()) + 100));
		iNum /= 100;
	}
#endif
	...
	if(iDifference > 0)
	{
		...
		if(eMajority != NO_RELIGION)
		{
			...
			if(pReligion)
			{
				...
				//modAcken code for follower beleifs
#if defined(AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_GROWTH)
				if(bAtPeace)
				{
					int iFollowerGrowthMod = 0;
					BeliefTypes eSecondaryFollowerOne = GetCityReligions()->GetSecondaryReligionFollowerBelief(false);
					BeliefTypes eSecondaryFollowerTwo = GetCityReligions()->GetSecondaryReligionFollowerBelief(true);
					if (eSecondaryFollowerOne != NO_BELIEF)
					{
						iFollowerGrowthMod += GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->GetCityGrowthModifier();
						if (eSecondaryFollowerTwo != NO_BELIEF)
						{
							iFollowerGrowthMod += GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->GetCityGrowthModifier();
						}
					}
					iReligionGrowthMod += iFollowerGrowthMod;
				}
#endif
				//endmod
...
int CvCity::GetJONSCultureThreshold() const
{
	...
	if(eMajority != NO_RELIGION)
	{
		...
		if(pReligion)
		{
			...
			BeliefTypes eSecondaryPantheon = GetCityReligions()->GetSecondaryReligionPantheonBelief();
			//NoteAcken No follower beleifs are supposed to increase border expansion
			if (eSecondaryPantheon != NO_BELIEF)
			{
...
#ifdef AMB_LESS_BORDER_CS
	if(GET_PLAYER(getOwner()).isMinorCiv())
	{
		iCultureThreshold *= AMB_LESS_BORDER_CS;
	}
#endif
...
int CvCity::GetLocalHappiness() const
{
	...
	if(eMajority != NO_RELIGION)
	{
		...
		if(pReligion)
		{
			...
			//NoteAcken No religious follower beleifs increase happiness on rivers or per cities
			if (eSecondaryPantheon != NO_BELIEF && getPopulation() >= GC.GetGameBeliefs()->GetEntry(eSecondaryPantheon)->GetMinPopulation())
			{
			...
			for(int jJ = 0; jJ < GC.getNumBuildingClassInfos(); jJ++)
			{
				...
				if(eBuilding != NO_BUILDING)
				{
					if(GetCityBuildings()->GetNumBuilding(eBuilding) > 0)
					{
						...
						//modAcken code for follower beleifs. Note that no pantheon are supposed to increase building happiness
#if defined(AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_HAPPINESS)
						BeliefTypes eSecondaryFollowerOne = GetCityReligions()->GetSecondaryReligionFollowerBelief(false);
						BeliefTypes eSecondaryFollowerTwo = GetCityReligions()->GetSecondaryReligionFollowerBelief(true);
						//int iIDBuilding = GC.getBuildingClassInfo(eBuildingClass)->GetID();
						if (eSecondaryFollowerOne != NO_BELIEF && iFollowers >= GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->GetMinFollowers())
						{
							iHappinessFromReligion += GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->GetBuildingClassHappiness(eBuildingClass);
							if (eSecondaryFollowerTwo != NO_BELIEF && iFollowers >= GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->GetMinFollowers())
							{
								iHappinessFromReligion += GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->GetBuildingClassHappiness(eBuildingClass);
							}
						}
#endif
						//endmod
...
BuildingTypes CvCity::ChooseFreeDefenseBuilding() const
{
	BuildingTypes eRtnValue = NO_BUILDING;
	int iNumBuildingInfos = GC.getNumBuildingInfos();
	CvWeightedVector<int, SAFE_ESTIMATE_NUM_BUILDINGS, true> buildingChoices;
	for(int iI = 0; iI < iNumBuildingInfos; iI++)
	{
		...
		if(pkBuildingInfo)
		{
			...
			if(!isWorldWonderClass(kBuildingClassInfo) && !isNationalWonderClass(kBuildingClassInfo))
			{
				int iDefense = pkBuildingInfo->GetDefenseModifier();
				int iCost = pkBuildingInfo->GetProductionCost();
				if(getFirstBuildingOrder(eBuilding) != -1 || canConstruct(eBuilding))
				{
					if(iDefense > 0 && iCost > 0)
					{
						int iWeight = iDefense * 10000 / iCost;
...
#ifdef AMB_POLICY_FREEDEFENSE
//	--------------------------------------------------------------------------------
bool CvCity::IsOwedDefenseBuilding() const
{
	return m_bOwedDefenseBuilding;
}
//	--------------------------------------------------------------------------------
void CvCity::SetOwedDefenseBuilding(bool bNewValue)
{
	m_bOwedDefenseBuilding = bNewValue;
}
#endif
//	--------------------------------------------------------------------------------
...
int CvCity::GetWLTKDNoResource() const
{
	VALIDATE_OBJECT
	return m_iWLTKDNoResource;
}

//	--------------------------------------------------------------------------------
///Sets number of turns left in WLTKD
void CvCity::SetWLTKDNoResource(int iValue)
{
	VALIDATE_OBJECT
	m_iWLTKDNoResource = iValue;
}

//	--------------------------------------------------------------------------------
///Changes number of turns left in WLTKD
void CvCity::ChangeWLTKDNoResource(int iChange)
{
	VALIDATE_OBJECT
	SetWLTKDNoResource(GetWLTKDNoResource() + iChange);
}
...
int CvCity::getBaseYieldRateModifier(YieldTypes eIndex, int iExtra, CvString* toolTipSink) const
{
	...
	if(pReligion)
	{
		...
		//modAcken code for follower beleifs. Note that no pantheon are supposed to increase building happiness
#if defined(AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_MAXYIELDFOLLOWERS)
		iTempMod = 0;
		BeliefTypes eSecondaryFollowerOne = GetCityReligions()->GetSecondaryReligionFollowerBelief(false);
		BeliefTypes eSecondaryFollowerTwo = GetCityReligions()->GetSecondaryReligionFollowerBelief(true);
		ReligionTypes eSecondary = GetCityReligions()->GetSecondaryReligion();
		int iSecondaryReligionYieldMaxFollowers = 0;
		if (eSecondaryFollowerOne != NO_BELIEF)
		{
			iSecondaryReligionYieldMaxFollowers += GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->GetMaxYieldModifierPerFollower(eIndex);
		 	if (iSecondaryReligionYieldMaxFollowers > 0)
			{
				int iFollowers = GetCityReligions()->GetNumFollowers(eSecondary);
				iTempMod += min(iFollowers, iSecondaryReligionYieldMaxFollowers);
			}
		}
		if (eSecondaryFollowerTwo != NO_BELIEF)
		{
			iSecondaryReligionYieldMaxFollowers += GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->GetMaxYieldModifierPerFollower(eIndex);
			if (iSecondaryReligionYieldMaxFollowers > 0)
			{
				int iFollowers = GetCityReligions()->GetNumFollowers(eSecondary);
				iTempMod += min(iFollowers, iSecondaryReligionYieldMaxFollowers);
		...
		if (iReligionYieldMaxFollowers > 0 || iSecondaryReligionYieldMaxFollowers > 0)
		{
...
#else
		int iFollowers = GetCityReligions()->GetNumFollowers(eMajority);
		if (iReligionYieldMaxFollowers > 0) //original
		{
			iTempMod = min(iFollowers, iReligionYieldMaxFollowers);
			iModifier += iTempMod;
			if(toolTipSink)
...
#ifdef AMB_BELIEF_CITYSTRENGTH
	ReligionTypes eMajority = GetCityReligions()->GetReligiousMajority();
	int iReligionBonus = 0;
	if(eMajority > NO_RELIGION)
	{
		iReligionBonus += GC.getGame().GetGameReligions()->GetReligion(eMajority,getOwner())->m_Beliefs.GetCityStrengthModifier();
		//Here I'm assuming there is only one such bonus ?
		if(iReligionBonus==0)
		{
			BeliefTypes eSecondaryPantheon = GetCityReligions()->GetSecondaryReligionPantheonBelief();
			if (eSecondaryPantheon != NO_BELIEF)
			{
				iReligionBonus += GC.GetGameBeliefs()->GetEntry(eSecondaryPantheon)->GetCityStrengthModifier();
			}
		}
	}
	m_iStrengthValue += iReligionBonus;
#endif
#ifdef AMB_POLICY_IDEOLOGIES
	m_iStrengthValue += GET_PLAYER(getOwner()).GetPlayerPolicies()->GetNumericModifier(POLICYMOD_CITYSTRENGTHMODIFIER);
#endif
	DLLUI->setDirty(CityInfo_DIRTY_BIT, true);
}
	...
	if(bForRangeStrike)
	{
		...
		if(eMajority != NO_RELIGION)
		{
			...
			if(pReligion)
			{
				...
				BeliefTypes eSecondaryPantheon = GetCityReligions()->GetSecondaryReligionPantheonBelief();
				//NoteAcken no follower beleifs change that
				if (eSecondaryPantheon != NO_BELIEF)
				{
...
#ifdef AMB_CITY_WEAKER_RANGE
		//AMB_LOG("City attack Was %i", iValue);
		//iValue *= (100 - AMB_CITY_WEAKER_RANGE);
		//iValue /= 100;
		//AMB_LOG("City attack is %i", iValue);
#endif
		return iValue;
	}
	...
	case YIELD_FAITH:
	{
			...
			if(pkUnitInfo)
			{
...
#ifdef AMB_CIV_UNITFAITH
				if(pkUnitInfo->IsUniqueBuyableWithFaith())
				{
						if(!canTrain(eUnitType, false, !bTestTrainable, false /*bIgnoreCost*/, true /*bWillPurchase*/))
						{
							return false;
				...
				if (pkUnitInfo->IsRequiresFaithPurchaseEnabled())
				{
					...
					if (ePrereqTech == -1)
					{
						...
						{
#if defined(AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_FAITHUNITS)
							//modAcken Purchase units with faith second religion that dont require a technology
							BeliefTypes eSecondaryFollowerOne=GetCityReligions()->GetSecondaryReligionFollowerBelief(false);
							BeliefTypes eSecondaryFollowerTwo=GetCityReligions()->GetSecondaryReligionFollowerBelief(true);
							//if no secondary stuff, false
							if(eSecondaryFollowerOne==NO_BELIEF)
							{
									return false;
							}
							//if that beleif allow nothing verify the second one
							else if(!GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->IsFaithUnitPurchaseEra((EraTypes)0))
							{
								if(eSecondaryFollowerTwo==NO_BELIEF)
								{
									return false;
								}
								else if(!GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->IsFaithUnitPurchaseEra((EraTypes)0))
								{
									return false; 
								}
							}
					...
					else
					{
						...
						else
						{
							...
							{
#if defined(AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_FAITHUNITS)
								//modAcken Purchase units with faith second religion
								BeliefTypes eSecondaryFollowerOne=GetCityReligions()->GetSecondaryReligionFollowerBelief(false);
								BeliefTypes eSecondaryFollowerTwo=GetCityReligions()->GetSecondaryReligionFollowerBelief(true);
								//if no secondary stuff, false
								if(eSecondaryFollowerOne==NO_BELIEF)
								...
								//if that beleif allow nothing verify the second one
								else if(!GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->IsFaithUnitPurchaseEra((EraTypes)pkTechInfo->GetEra()))
								{
									if(eSecondaryFollowerTwo==NO_BELIEF)
									{
										return false;
									}
									else if(!GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->IsFaithUnitPurchaseEra((EraTypes)pkTechInfo->GetEra()))
									{
										return false; 
									}
								}
		...
		else if(eBuildingType != NO_BUILDING)
		{
			...
			if(pkBuildingInfo && pkBuildingInfo->IsUnlockedByBelief())
			{
				...
				{
#if defined(AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_FAITHBUILDINGS)
					//modAcken Purchase units with faith second religion
					BeliefTypes eSecondaryFollowerOne=GetCityReligions()->GetSecondaryReligionFollowerBelief(false);
					BeliefTypes eSecondaryFollowerTwo=GetCityReligions()->GetSecondaryReligionFollowerBelief(true);
					//if no secondary stuff, false
					...
					//if that beleif allow nothing verify the second one
					else if(!GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->IsBuildingClassEnabled((BuildingClassTypes)pkBuildingInfo->GetBuildingClassType()))
					{
						if(eSecondaryFollowerTwo==NO_BELIEF)
						{
							return false;
						}
						else if(!GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->IsBuildingClassEnabled((BuildingClassTypes)pkBuildingInfo->GetBuildingClassType()))
						{
							return false;
						}
					}
...
#ifdef AMB_POLICY_BUILDINGGOLDTOCULTURE
			if(bResult)
			{
				int iBonusCulture = iGoldCost * GET_PLAYER(getOwner()).GetPlayerPolicies()->GetNumericModifier(POLICYMOD_BUILDING_GOLD_TO_CULTURE);
				iBonusCulture /=100;
				if(iBonusCulture > 0)
				{
					GET_PLAYER(getOwner()).changeJONSCulture(iBonusCulture);
					if (plot() && plot()->GetActiveFogOfWarMode() == FOGOFWARMODE_OFF)
					{
						char text[256] = {0};
						sprintf_s(text, "[COLOR_MAGENTA]+%d[ENDCOLOR][ICON_CULTURE]", iBonusCulture);
						float fDelay = GC.getPOST_COMBAT_TEXT_DELAY() * 1;
...
#ifdef AMB_POLICY_FREEDEFENSE
	kStream >> m_bOwedDefenseBuilding;
#endif
