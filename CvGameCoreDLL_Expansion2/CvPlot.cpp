...
int CvPlot::calculateNatureYield(YieldTypes eYield, TeamTypes eTeam, bool bIgnoreFeature) const
{
	...
	if(!bIgnoreFeature)
	{
		if(getFeatureType() != NO_FEATURE)
		{
			...
			{
				iYield = iYieldChange;
#ifdef AMB_AURORA_FIX
				// Extra yield for religion on this terrain
				if(pWorkingCity != NULL && eMajority != NO_RELIGION)
				{
					const CvReligion* pReligion2 = GC.getGame().GetGameReligions()->GetReligion(eMajority, pWorkingCity->getOwner());
					if(pReligion2)
					{
						int iReligionChange2 = pReligion2->m_Beliefs.GetTerrainYieldChange(getTerrainType(), eYield);
						if (eSecondaryPantheon != NO_BELIEF)
						{
							iReligionChange2 += GC.GetGameBeliefs()->GetEntry(eSecondaryPantheon)->GetTerrainYieldChange(getTerrainType(), eYield);
						}
						iYield += iReligionChange2;
					}
				}
#endif
			}
