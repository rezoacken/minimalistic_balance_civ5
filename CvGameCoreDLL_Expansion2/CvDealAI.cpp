...
#ifdef AMB_AI_DEAL_WHITE_PEACE
	if (pDeal->GetGoldTrade(eOtherPlayer) == 0 && pDeal->GetGoldTrade(m_pPlayer->GetID()) == 0 && !pDeal->IsPeaceTreatyTrade(eOtherPlayer))
#else
...
int CvDealAI::GetGPTforForValueExchange(int iGPTorValue, bool bNumGPTFromValue, int iNumTurns, bool bFromMe, PlayerTypes eOtherPlayer, bool bUseEvenValue, bool bRoundUp)
{
	...
	if(bNumGPTFromValue)
	{
		...
		if(bFromMe)
		{
			iMultiplier = GD_INT_GET(AMB_AI_DEAL_HUMAN_INTEREST);
		}
		else
		{
			iMultiplier = GD_INT_GET(AMB_AI_DEAL_AI_INTEREST);
		}
		iDivisor = 100;
#else
		iMultiplier = 100;
		iDivisor = /*85*/ GC.getEACH_GOLD_PER_TURN_VALUE_PERCENT();
#endif
	...
	else
	{
		...
		if(bFromMe)
		{
			iDivisor = GD_INT_GET(AMB_AI_DEAL_AI_INTEREST);
		}
		else
		{
			iDivisor = GD_INT_GET(AMB_AI_DEAL_HUMAN_INTEREST);
		}
		iMultiplier = 100;
#else
	...
	if(bFromMe)
	{
#ifndef AMB_AI_DEAL_LOGICAL_VALUES
...
int CvDealAI::GetResourceValue(ResourceTypes eResource, int iResourceQuantity, int iNumTurns, bool bFromMe, PlayerTypes eOtherPlayer)
{
	...
	if(eUsage == RESOURCEUSAGE_LUXURY)
	{
		...
		else
		{
			...
			/*if(!bFromMe)
			{
				int iHappinessPlayer = GetPlayer()->GetExcessHappiness();
				int iReduction = min(4,max(0,iHappinessPlayer/10))*25;
				iItemValue *= (100-iReduction);
				iItemValue /= 100;
			}
			*/
#endif
...
#ifdef WIP_AMB_AI_DEAL_VALUE_RESSOURCES
			else
			{
				//If I dont have it value it higher if I can get a king day with it
				if(GetPlayer()->getNumResourceAvailable(eResource) == 0)
				{
					...
					{
						if(pLoopCity->GetResourceDemanded(true)==eResource)
						{
							iItemValue *= 3;
						}
					}
...
#else
			iItemValue += (iResourceQuantity * iNumTurns * 150 / 100);	// Ex: 5 Iron for 30 turns * 1.5 = value of 225
#endif
		}
		else
		{
...
int CvDealAI::GetThirdPartyWarValue(bool bFromMe, PlayerTypes eOtherPlayer, TeamTypes eWithTeam)
{
...
#ifdef AMB_AI_DEAL_VALUE_WAR
	//Still play nice if we're deceptive ?
	MajorCivApproachTypes eMajorApproachAskingPlayer = pDiploAI->GetMajorCivApproach(eOtherPlayer,false);
	MajorCivApproachTypes eMajorApproachWarPlayer = NO_MAJOR_CIV_APPROACH;
#endif
	...
	{
		eOpinionTowardsWarPlayer = pDiploAI->GetMajorCivOpinion(eWithPlayer);
		eMajorApproachWarPlayer = pDiploAI->GetMajorCivApproach(eWithPlayer,false);
	}
#else
		eOpinionTowardsWarPlayer = pDiploAI->GetMajorCivOpinion(eWithPlayer);
#endif
	...
	if(bFromMe)
	{
		...
		else
		{
			...
			//Modify for our current approach toward the player
			int iApproachModWarPlayer = 0;
			switch(eMajorApproachWarPlayer)
			{
			case MAJOR_CIV_APPROACH_WAR:
				iApproachModWarPlayer = -50;
				break;
			case MAJOR_CIV_APPROACH_HOSTILE:
				iApproachModWarPlayer = -35;
				break;
			case MAJOR_CIV_APPROACH_DECEPTIVE:
				iApproachModWarPlayer = -35;
				break;
			case MAJOR_CIV_APPROACH_GUARDED:
				iApproachModWarPlayer = -15;
				break;
			case MAJOR_CIV_APPROACH_AFRAID:
				iApproachModWarPlayer = 100;
				break;
			case MAJOR_CIV_APPROACH_FRIENDLY:
				iApproachModWarPlayer = 100;
				break;
			case MAJOR_CIV_APPROACH_NEUTRAL:
				iApproachModWarPlayer = 25;
				break;	
			}
			iWarMod *= (100+iApproachModWarPlayer)*(100+iOpinionModWarPlayer);
			iWarMod /= 100;
			iWarMod /= 100;
		}
		// Modify for our feelings towards the asking player
		int iOpinionModAskingPlayer = 0;
		switch(eOpinionTowardsAskingPlayer) 
		{
		case MAJOR_CIV_OPINION_ALLY:
			iOpinionModAskingPlayer = -25;
			break;
		case MAJOR_CIV_OPINION_FRIEND:
			iOpinionModAskingPlayer = -15;
			break;
		case MAJOR_CIV_OPINION_FAVORABLE:
			iOpinionModAskingPlayer = -5;
			break;
		case MAJOR_CIV_OPINION_COMPETITOR:
			iOpinionModAskingPlayer = 5;
			break;
		...
		int iApproachModAskingPlayer = 0;
		switch(eMajorApproachAskingPlayer)
		{
		case MAJOR_CIV_APPROACH_WAR:
		case MAJOR_CIV_APPROACH_DECEPTIVE:
			iApproachModAskingPlayer = 50;
			if(pDiploAI->GetWarProjection(eOtherPlayer)>WAR_PROJECTION_UNKNOWN)
			{
				iApproachModAskingPlayer = 100;
			}
			break;
		case MAJOR_CIV_APPROACH_HOSTILE:
			iApproachModAskingPlayer = 25;
			break;
		case MAJOR_CIV_APPROACH_GUARDED:
			iApproachModAskingPlayer = 15;
			break;
		case MAJOR_CIV_APPROACH_AFRAID:
			iApproachModAskingPlayer = -25;
			break;
		...
		iWarMod *= (100+iApproachModAskingPlayer)*(100+iOpinionModAskingPlayer);
		iWarMod /= 100;
		iWarMod /= 100;

		iItemValue *= iWarMod;
		iItemValue /= 100;
		AMB_LOG("War bribe Value %i, Mod %i",iItemValue,iWarMod);
	}
#else
			// Modify for our feelings towards the player we're would go to war with
			if(eOpinionTowardsWarPlayer == MAJOR_CIV_OPINION_UNFORGIVABLE)
...
#ifdef AMB_AI_DEAL_PEACE_BASED_ON_DISTANCE
	PlayerProximityTypes eProximity = GetPlayer()->GetProximityToPlayer(eOtherPlayer);
	switch (eProximity)
	{
	case PLAYER_PROXIMITY_DISTANT:
		eTreaty =(PeaceTreatyTypes)max(0,(int)eTreaty-2);
		break;
	case PLAYER_PROXIMITY_FAR:
		eTreaty = (PeaceTreatyTypes)max(0,(int)eTreaty-1);
		break;
	}
#endif
