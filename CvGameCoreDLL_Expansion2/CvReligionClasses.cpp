...
int CvPlayerReligions::GetCostNextProphet(bool bIncludeBeliefDiscounts, bool bAdjustForSpeedDifficulty) const
{
...
#if defined(AMB_POLICY_MANDATEOFHEAVEN_PROPHETCOST)
	//modAck Mandate of heaven on prophet cost
	int iProphetCostMod = m_pPlayer->GetPlayerPolicies()->GetNumericModifier(POLICYMOD_FAITH_COST_MODIFIER); //modAcken added policy faith cost modifiers
	//endmod
#endif
	...
	if (bIncludeBeliefDiscounts && ePlayerReligion > RELIGION_PANTHEON)
	{
		...
		if(pReligion)
		{
			...
			iProphetCostMod += pReligion->m_Beliefs.GetProphetCostModifier(); //modAcken
...
BeliefTypes CvCityReligions::GetSecondaryReligionPantheonBelief()
{
	...
	///ModAcken Is there a follower belief in the secondary religion here? Returns both
	BeliefTypes CvCityReligions::GetSecondaryReligionFollowerBelief(bool bSecondBelief)
	{
		BeliefTypes eRtnValue = NO_BELIEF;
			...
			if (eSecondary != NO_RELIGION)
			{
				...
				if(pReligion)
				{
					for(int iI = 0; iI < pReligion->m_Beliefs.GetNumBeliefs(); iI++)
					{
						...
						CvBeliefEntry* pEntry = GC.GetGameBeliefs()->GetEntry((int)eBelief);
						if(pEntry && pEntry->IsFollowerBelief())
						{	//Is it the first Follower Belief or the second ?
							if(!bSecondBelief)
							{
								eRtnValue = eBelief;
								break;
							...
							{	//Found the first so the second is next
								bSecondBelief=false;
							}
						}
					}
...
#ifdef AMB_AI_RESPECT_NO_SPREAD
						if (!GET_PLAYER(pLoopCity->getOwner()).isMinorCiv())
						{
							if (m_pPlayer->GetDiplomacyAI()->IsPlayerAgreeNotToConvert(pLoopCity->getOwner()))
							{
								iScore = 0;
							}
						}
...
#ifdef AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_FAITHBUILDINGS
		else if(m_pPlayer->IsSecondReligionPantheon() && BuyAnyAvailableFaithBuilding(true))
		{
			AMB_LOG("Bought secondary religion building");
		}
#endif
...
#ifdef AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_FAITHBUILDINGS
bool CvReligionAI::BuyAnyAvailableFaithBuilding(bool bSecondaryReligion)
#else
...
#ifdef AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_FAITHBUILDINGS
		ReligionTypes eReligion;
		if(!bSecondaryReligion)
		{
			eReligion = pLoopCity->GetCityReligions()->GetReligiousMajority();
		}
		else
		{
			eReligion = pLoopCity->GetCityReligions()->GetSecondaryReligion();
		}
#else
		ReligionTypes eReligion = pLoopCity->GetCityReligions()->GetReligiousMajority();
#endif
...

{
	...
	iRtnValue += int((pEntry->GetCityStrengthModifier())/10 * (double)MIN(iFlavorCityDefense, iFlavorDefense) / 30.0+0.5);
#endif
	...
	iRtnValue += int((pEntry->GetSettlerProductionModifier()) * (1.0 + log10((double)iFlavorExpansion) / 10.0) / 10.0 + 0.5);
#endif
...
int CvReligionAI::ScoreBeliefForPlayer(CvBeliefEntry* pEntry)
{
...
#ifdef AMB_MOD_H
		int iTemp = pEntry->GetFaithFromKills() * pEntry->GetMaxDistance() * iFlavorOffense / 150;
#else
	...
	if (pEntry->GetFaithBuildingTourism() > 0)
	{
		...
		iRtnValue += (pEntry->GetFaithBuildingTourism() * 20) * FaithBuildingAvailable(eReligion).size();
#else
