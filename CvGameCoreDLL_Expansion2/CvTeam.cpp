			...
			for(int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
			{
				...
				if(GET_PLAYER(eLoopPlayer).getTeam() == GetID())
				{
					...
					for(pLoopCity = GET_PLAYER(eLoopPlayer).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER(eLoopPlayer).nextCity(&iLoop))
					{
						...
						if (pLoopCity->IsOwedDefenseBuilding())
						{
							BuildingTypes eFreeDefenseBuilding = pLoopCity->ChooseFreeDefenseBuilding();
							if (eFreeDefenseBuilding != NO_BUILDING)
							{
								pLoopCity->GetCityBuildings()->SetNumFreeBuilding(eFreeDefenseBuilding, 1);
								pLoopCity->SetOwedDefenseBuilding(false);
							}
						}
#endif
					}
