...
#ifdef AMB_DIPLOAI_CAN_ATTACK_MULTIPLE_MAJOR_GS_CONQUEST
bool CvMilitaryAI::RequestSneakAttack(PlayerTypes eEnemy, bool bAdditionalTarget)
#else
...
#endif
{
...
#ifdef AMB_DIPLOAI_CAN_ATTACK_MULTIPLE_MAJOR_GS_CONQUEST
	if (m_pPlayer->haveAIOperationOfType(AI_OPERATION_NAVAL_SNEAK_ATTACK, &iOperationID) && !bAdditionalTarget)
	{
		return false;
	}
	if (m_pPlayer->haveAIOperationOfType(AI_OPERATION_SNEAK_CITY_ATTACK, &iOperationID) && !bAdditionalTarget)
	{
		return false;
	}
#else
	...
	{
		AMB_LOG("Targeting City. FindBestAttackTarget");
	}
#endif
	...
	if(target.m_pTargetCity)
	{
		...
		if(bAdditionalTarget)
		{
			if(target.m_pTargetCity)
			{
			AMB_LOG("Target acquired: %s",target.m_pTargetCity->getName().c_str());
			}
		}
#endif
		...
		if(target.m_bAttackBySea)
		{
			...
			if(IsAttackReady(MUFORMATION_NAVAL_INVASION, AI_OPERATION_NAVAL_SNEAK_ATTACK))
			{
				...
				if(pOperation != NULL && !pOperation->ShouldAbort())
				{
					...
					if(bAdditionalTarget)
					{
						AMB_LOG("Turn %i Additional SNEAK_ATTACK_NAVAL registered against %s",GC.getGame().getGameTurn(),GET_PLAYER(eEnemy).getName());
					}
#endif
		...
		else
		{
			...
			if(IsAttackReady((GC.getGame().getHandicapInfo().GetID() > 3 && !(GC.getMap().GetAIMapHint() & 1)) ? MUFORMATION_BIGGER_CITY_ATTACK_FORCE : MUFORMATION_BASIC_CITY_ATTACK_FORCE, AI_OPERATION_SNEAK_CITY_ATTACK))
#else
...
#endif
			{
				...
				if(pOperation != NULL && !pOperation->ShouldAbort())
				{
					...
					if(bAdditionalTarget)
					{
						AMB_LOG("Turn %i Additional SNEAK_ATTACK_LAND registered against %s",GC.getGame().getGameTurn(),GET_PLAYER(eEnemy).getName());
					}
#endif
...
bool CvMilitaryAI::RequestBasicAttack(PlayerTypes eEnemy, int iNumUnitsWillingBuild)
{
	...
	if(!target.m_pTargetCity)
		AMB_LOG("Target fail");
	//AMB_LOG("Best target %s, request", target.m_pTargetCity->getName().c_str());
#endif
...
bool CvMilitaryAI::RequestSpecificAttack(CvMilitaryTarget kTarget, int iNumUnitsWillingToBuild)
{
	...
	if(kTarget.m_pTargetCity)
	{
		...
		{
#ifdef AMB_DIFFICULTY_ID_ADJUSTMENT
			iFilledSlots = MilitaryAIHelpers::NumberOfFillableSlots(m_pPlayer,(GC.getGame().getHandicapInfo().GetID() > 3 && !(GC.getMap().GetAIMapHint() & 1)) ? MUFORMATION_BIGGER_CITY_ATTACK_FORCE : MUFORMATION_BASIC_CITY_ATTACK_FORCE, false, &iNumRequiredSlots, &iLandReservesUsed);
#else
...
#endif
#ifdef AMB_DIPLOAI_CAN_ATTACK_MULTIPLE_MAJOR_GS_CONQUEST
			AMB_LOG("%i - %i <= %i et %i <= %i",iNumRequiredSlots,iFilledSlots,iNumUnitsWillingToBuild, iLandReservesUsed,GetLandReservesAvailable());
#endif
			if((iNumRequiredSlots - iFilledSlots) <= iNumUnitsWillingToBuild && iLandReservesUsed <= GetLandReservesAvailable())
			{
				AMB_LOG("addOperation");
...
CvMilitaryTarget CvMilitaryAI::FindBestAttackTarget(AIOperationTypes eAIOperationType, PlayerTypes eEnemy, int* piWinningScore)
{
...
#ifdef AUI_MILITARY_ALWAYS_TARGET_CAPITALS
	int iCapitalCount = 0;
#endif
#ifdef AMB_AI_PICKBESTTARGET_USES_PATHFINDER
	int iUnit;
	//CvUnit* pLoopUnit;
	for(pLoopUnit = m_pPlayer->firstUnit(&iUnit); pLoopUnit != NULL; pLoopUnit = m_pPlayer->nextUnit(&iUnit))
	{
		if(!pLoopUnit->ignoreTerrainCost() && pLoopUnit->getDomainType()==DOMAIN_LAND && pLoopUnit->IsCombatUnit()&& pLoopUnit->baseMoves(DOMAIN_LAND)==2)
		{
			//AMB_LOG("Found a unit %s",pLoopUnit->getName().c_str());
			break;
		}
	}
#endif
	for(pFriendlyCity = m_pPlayer->firstCity(&iFriendlyLoop); pFriendlyCity != NULL; pFriendlyCity = m_pPlayer->nextCity(&iFriendlyLoop))
	{
		for(pEnemyCity = GET_PLAYER(eEnemy).firstCity(&iEnemyLoop); pEnemyCity != NULL; pEnemyCity = GET_PLAYER(eEnemy).nextCity(&iEnemyLoop))
		{
			if(pEnemyCity->plot()->isRevealed(m_pPlayer->getTeam()))
			{
...
#ifdef AMB_AI_PICKBESTTARGET_USES_PATHFINDER
					//experimental maybe time consuming
					if(!target.m_bAttackBySea)
					{
						...
						if(pLoopUnit)
						{
							GC.getIgnoreUnitsPathFinder().SetData(pLoopUnit);
							if (GC.getIgnoreUnitsPathFinder().GeneratePath(target.m_pMusterCity->getX(), target.m_pMusterCity->getY(), target.m_pTargetCity->getX(), target.m_pTargetCity->getY(),MOVE_UNITS_THROUGH_ENEMY|MOVE_IGNORE_STACKING|MOVE_UNITS_IGNORE_DANGER,true))
							{
								pNode = GC.getIgnoreUnitsPathFinder().GetLastNode();
							}
							if (pNode)
							{	
								iDistance = pNode->m_iData2;
							if (iDistance < MAX_INT)
								iDistance *= pLoopUnit->baseMoves(DOMAIN_LAND);
								//AMB_LOG("Old was:%i",target.m_iPathLength);
								target.m_iPathLength = iDistance;
								
							}
							//AMB_LOG("New is %i for %s to %s",iDistance,target.m_pMusterCity->getName().c_str(),target.m_pTargetCity->getName().c_str());
						}
					}
#endif
				}
				...
				// capitals are always added to the list if conquest victory enabled
				if (eVictory != NO_VICTORY && GC.getGame().isVictoryValid(eVictory))
				{
					...
					{
						// proximity to enemy capital is also considered
						if (target.m_iPathLength > 0)
						{
							if(!GET_PLAYER(eEnemy).isMinorCiv())
							{
								iWeight = (10000 - MIN(target.m_iPathLength, plotDistance(GET_PLAYER(eEnemy).getCapitalCity()->getX(), GET_PLAYER(eEnemy).getCapitalCity()->getY(), target.m_pTargetCity->getX(), target.m_pTargetCity->getY())));
							}
							else
							{
				...
				else if (target.m_iPathLength > 0)
				{
...
#endif // AUI_MILITARY_ALWAYS_TARGET_CAPITALS
			}
		}
	}
	...
	// If running conquest victory, capitals are counted for free (ie. not included in the 25)

	prelimWeightedTargetList.SortItems();
	weightedTargetList.clear();
	int iTargetsConsidered = 0;
#ifdef AUI_MILITARY_ALWAYS_TARGET_CAPITALS
	for (int iI = 0; iI < prelimWeightedTargetList.size() && iTargetsConsidered < (25 + iCapitalCount); iI++)
#else
...
#ifdef AMB_DIPLOAI_CAN_ATTACK_MULTIPLE_MAJOR_GS_CONQUEST
/// How strong are the civs we're fighting
int CvMilitaryAI::GetMightCivsAtWarWith() const
{
	PlayerTypes eLoopPlayer;
	WarStateTypes eWarState;
	int iRtnValue = 0;
	...
	for(int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
	{
		...
		if(eLoopPlayer != m_pPlayer->GetID() && m_pPlayer->GetDiplomacyAI()->IsPlayerValid(eLoopPlayer))
		{
			...
			if(eWarState != NO_WAR_STATE_TYPE)
			{
				//AMB_LOG("%s at war with %s: %i", m_pPlayer->getName(), GET_PLAYER(eLoopPlayer).getNameKey(),GET_PLAYER(eLoopPlayer).GetMilitaryMight());
				iRtnValue += GET_PLAYER(eLoopPlayer).GetMilitaryMight();
			}
		}
	}
...
void CvMilitaryAI::UpdateBaseData()
{
...
#ifdef AMB_GS_CONQUEST_ARMY_SIZE_BOOST
	fMultiplier = (float)0.80 + (((float)(m_pPlayer->GetMilitaryAI()->GetHighestThreat() + iFlavorOffense + iFlavorDefense)) / (float)200.0);
#else
...
#ifdef AMB_DIFFICULTY_ID_ADJUSTMENT
	int iDifficulty = max(0,GC.getGame().getHandicapInfo().GetID());
#else
	...
	if (m_pPlayer->isMinorCiv())
	{
		...
		m_iMandatoryReserveSize = AMB_LESS_UNIT_CS;
		iNumUnitsWanted = 0;
#else
...
#ifdef AMB_GS_CONQUEST_ARMY_SIZE_BOOST
		if(!bNavalMap && m_pPlayer->GetDiplomacyAI()->IsGoingForWorldConquest())
		{
			iNumUnitsWanted *= 1 + max(0,(m_pPlayer->GetDiplomacyAI()->GetNumMajorCivApproach(MAJOR_CIV_APPROACH_WAR)+m_pPlayer->GetDiplomacyAI()->GetNumMajorCivApproach(MAJOR_CIV_APPROACH_DECEPTIVE)-1)/2);
		}
#endif 
...
#ifdef AMB_GS_CONQUEST_ARMY_SIZE_BOOST
		//The number of wanted units scales with era and how close to victory we are if we are using conquest GS
		//The total % increase is in AMB_GS_CONQUEST_ARMY_SIZE_BOOST
		//The % increase with More than half the capitals

		if(bConquestGrandStrategy)
		{
			//half the bonus from era (scales)
			double dEraBoost = GD_INT_GET(GS_CONQUEST_ARMY_SIZE_BOOST)/2;
			dEraBoost /= GC.getNumEraInfos();
			dEraBoost *= m_pPlayer->GetCurrentEra()+1;
			//half from the number of caps we have
			double dCapitalBoost =  GD_INT_GET(GS_CONQUEST_ARMY_SIZE_BOOST)/2;
			dCapitalBoost /= (GC.getGame().countMajorCivsEverAlive()-1);
			int iNumCapitals=0;
			for(int iMajorLoop = 0; iMajorLoop < MAX_MAJOR_CIVS; iMajorLoop++)
			{
				
				PlayerTypes ePlayer = (PlayerTypes) iMajorLoop;
				if(GET_PLAYER(ePlayer).GetCapitalConqueror()==m_pPlayer->GetID() && iMajorLoop != GetPlayer()->GetID() && !GET_PLAYER(ePlayer).isMinorCiv())
				{
						iNumCapitals += 1;
				}
			}
			dCapitalBoost *= iNumCapitals;
			dCapitalBoost /=100;
			dEraBoost /= 100;
			//AMB_LOG("%s army size was %i",m_pPlayer->getName(),iNumUnitsWanted);
			iNumUnitsWanted = (int)((float)iNumUnitsWanted * (1+dEraBoost+dCapitalBoost));
			//AMB_LOG("and is now %i",iNumUnitsWanted);
		}
#endif
...
#ifdef AUI_MILITARY_ROLL_FOR_NUKES_CONSIDER_NUKE_COUNT
							iFlavorNuke += m_pPlayer->getNumNukeUnits() - GET_PLAYER(eLoopPlayer).getNumNukeUnits();
#endif // AUI_MILITARY_ROLL_FOR_NUKES_CONSIDER_NUKE_COUNT
...
UnitTypes CvMilitaryAI::GetUnitForArmy(CvCity* pCity) const
{
	...
	{
#ifdef AMB_DIFFICULTY_ID_ADJUSTMENT
		eFormation = (GC.getGame().getHandicapInfo().GetID() > 3 && !(GC.getMap().GetAIMapHint() & 1)) ? MUFORMATION_BIGGER_CITY_ATTACK_FORCE : MUFORMATION_BASIC_CITY_ATTACK_FORCE;
#else
...
#ifndef AMB_AI_PRODUCTION_TWEAKS
...
#ifdef AMB_DIPLOAI_CAN_ATTACK_MULTIPLE_MAJOR_GS_CONQUEST
int MilitaryAIHelpers::TotalMight_AtWar(CvPlayer* pPlayer)
{
	return (pPlayer->GetMilitaryAI()->GetMightCivsAtWarWith());
}
#endif
...
bool MilitaryAIHelpers::IsTestStrategy_EradicateBarbarians(MilitaryAIStrategyTypes eStrategy, CvPlayer* pPlayer, int iBarbarianCampCount, int iVisibleBarbarianCount)
{
	...
	if(GC.getGame().getGameTurn() < 40*GC.getGame().getHandicapInfo().getResearchPercent())
	{
		return false;
	}
#endif
...
#if defined(AMB_AI_NAVAL_SIZE_DMULTIPLIER)
	dMultiplier = (double)0.50 + ((double)pPlayer->GetMilitaryAI()->GetHighestThreat() / (double)5.0) + ((double)(iFlavorNaval) / (double)50.0);//modAcken
#else
	dMultiplier = (double)0.75 + ((double)pPlayer->GetMilitaryAI()->GetHighestThreat() / (double)4.0) + ((double)(iFlavorNaval) / (double)40.0);//Original
#endif
