...
public:
	...
	RecklessExpanderTypes GetRecklessExpanderLevel(PlayerTypes ePlayer) const;
	void SetRecklessExpanderLevel(PlayerTypes ePlayer, RecklessExpanderTypes eRecklessExpander);
	void DoUpdateRecklessExpanderLevel();
#endif
#ifdef AMB_AI_WARMONGER
	int GetCasusBelliTimer(PlayerTypes ePlayer) const;
	void SetCasusBelliTimer(PlayerTypes ePlayer, int iTimer);
	void ChangeCasusBelliTimer(PlayerTypes ePlayer, int iChange);
	void DoDecayCasusBelliTimer();
	bool HasCasusBelliAgainst(PlayerTypes ePlayer) const;
	void SetCasusBelliAgainst(PlayerTypes ePlayer, bool bCasusBelli);
#endif
#ifdef AMB_AI_NO_ACTIVITY_WHITE_PEACE
	int GetWarNoCityTimer(PlayerTypes ePlayer) const;
	void SetWarNoCityTimer(PlayerTypes ePlayer, int iTimer);
	void ChangeWarNoCityTimer(PlayerTypes ePlayer, int iChange);
	int GetWarNoUnitTimer(PlayerTypes ePlayer) const;
	void SetWarNoUnitTimer(PlayerTypes ePlayer, int iTimer);
	void ChangeWarNoUnitTimer(PlayerTypes ePlayer, int iChange);
#endif
...
#ifdef AMB_AI_NO_ACTIVITY_WHITE_PEACE
	void DoIncreaseNoActivityTimers();
#endif
...
#ifdef	AMB_DIPLOAI_RECKLESS_EXPANDER_OVERHAUL
	void LogRecklessExpanderLevel(CvString& strString, PlayerTypes ePlayer);
#endif
...
#ifdef AMB_DIPLOAI_RECKLESS_EXPANDER_OVERHAUL
		char m_aeRecklessExpanderLevel[REALLY_MAX_PLAYERS];
#endif
#ifdef AMB_AI_WARMONGER
		int m_aiCasusBelliTimer[REALLY_MAX_PLAYERS];
		bool m_abHasCasusBelli[REALLY_MAX_PLAYERS];
#endif
#ifdef AMB_AI_NO_ACTIVITY_WHITE_PEACE
		int m_aiWarNoCityTimer[REALLY_MAX_PLAYERS];
		int m_aiWarNoUnitTimer[REALLY_MAX_PLAYERS];
#endif
...
#ifdef AMB_DIPLOAI_RECKLESS_EXPANDER_OVERHAUL
	char* m_paeRecklessExpanderLevel;
#endif
#ifdef AMB_AI_WARMONGER
	int* m_paiCasusBelliTimer;
	bool* m_pabHasCasusBelli;
#endif
#ifdef AMB_AI_NO_ACTIVITY_WHITE_PEACE
	int* m_paiWarNoCityTimer;
	int* m_paiWarNoUnitTimer;
#endif
