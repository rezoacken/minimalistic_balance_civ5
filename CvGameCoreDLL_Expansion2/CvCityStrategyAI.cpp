...
#include "LintFree.h"
#ifdef AMB_AI_PRODUCTION_UNIT_DEFENSLESS
...
void CvCityStrategyAI::FlavorUpdate()
{
	...
	for(int iFlavor = 0; iFlavor < GC.getNumFlavorTypes(); iFlavor++)
	{
		...
		{
			iFlavorValue = GET_PLAYER(m_pCity->getOwner()).GetGrandStrategyAI()->ScienceFlavorBoost() * MAX(1, iFlavorValue);
		}
...
void CvCityStrategyAI::ChooseProduction(bool bUseAsyncRandom, BuildingTypes eIgnoreBldg /* = NO_BUILDING */, UnitTypes eIgnoreUnit/*  = NO_UNIT */)
{
...
#ifdef AMB_DIFFICULTY_ID_ADJUSTMENT
		int iBonusMultiplier = min(2,max(1,GC.getGame().getHandicapInfo().GetID() - 4));
#else
	...
	for(iBldgLoop = 0; iBldgLoop < GC.GetGameBuildings()->GetNumBuildings(); iBldgLoop++)
	{
		...
		if(iBldgLoop != eIgnoreBldg && m_pCity->canConstruct(eLoopBuilding))
		{
			...
			/*NEW*/
			if(kPlayer.GetPlayerPolicies()->GetBuildingClassProductionModifier((BuildingClassTypes)kBuildingClassInfo.GetID()) > 40)
			{
				iTempWeight *= 10;
			}
			/**/
			if(pkBuildingInfo->GetSpecialistCount() > 1 || isNationalWonderClass(kBuildingClassInfo))
			{
				if(GetCity()->getPopulation() < 6)
				{
					iTempWeight /= 2;
				}
			}
			else if(!isWorldWonderClass(kBuildingClassInfo) && (GetCity()->getPopulation() < 8) && (pkBuildingInfo->GetFlavorValue((FlavorTypes) GC.getInfoTypeForString("FLAVOR_GROWTH")) > 1))
			{
				iTempWeight *=2;
			}
			else if(!isWorldWonderClass(kBuildingClassInfo) && (GetCity()->getPopulation() >= 8) && (pkBuildingInfo->GetFlavorValue((FlavorTypes) GC.getInfoTypeForString("FLAVOR_SCIENCE")) > 1))
			{
				iTempWeight *=2;
			}

			if(!GetCity()->isCapital() && pkBuildingInfo->GetFlavorValue((FlavorTypes) GC.getInfoTypeForString("FLAVOR_TOURISM")) > 50)
			{
				iTempWeight = 0;
			}

			//Boost slot buildings if player has no free slot
			//what sort of slot this building provide
			GreatWorkSlotType eWorkSlot = pkBuildingInfo->GetGreatWorkSlotType();
			if(eWorkSlot != NO_GREAT_WORK_SLOT && kPlayer.GetCulture()->GetNumAvailableGreatWorkSlots(eWorkSlot) == 0)
			{
				//Do we have a unit waiting for a slot ?
				int iLoop;
				for(CvUnit* pLoopUnit = kPlayer.firstUnit(&iLoop); pLoopUnit; pLoopUnit = kPlayer.nextUnit(&iLoop))
				{
					//skip if this unit is not a great person that can make a great work or is not waiting
					GreatWorkType eGW = pLoopUnit->GetGreatWork();
					if(eGW == NO_GREAT_WORK || pLoopUnit->GetGreatPeopleDirective() != NO_GREAT_PEOPLE_DIRECTIVE_TYPE)
					{
						continue;
					}
					//Is there a match ?
					GreatWorkSlotType eGreatWorkSlot = CultureHelpers::GetGreatWorkSlot(eGW);
					if(eGreatWorkSlot == eWorkSlot)
					{
						AMB_LOG("%s turn %i, building %s, Boosting culture %i",GetCity()->getName().c_str(),GC.getGame().getGameTurn(), pkBuildingInfo->GetTextKey(), iTempWeight);
						iTempWeight *= 3;
						AMB_LOG("Slot to fill");
			        ...
			        m_pCity->canTrain((UnitTypes)iUnitLoop))
			{
				...
				if(pkUnitEntry && pkUnitEntry->GetDefaultUnitAIType() == UNITAI_SETTLE)
				{
					...
					else
					{
...
#ifdef AMB_AI_NUM_CITIES_TWEAKS
						int iSettlersOnMap = kPlayer.GetNumUnitsWithUnitAI(UNITAI_SETTLE, true);
						if((kPlayer.getNumCities()+iSettlersOnMap) < 4)
						{
							int iTime = 50;
							iTime *= GC.getGame().getGameSpeedInfo().getConstructPercent();
							iTime /= 100;
							if(GC.getGame().getGameTurn() > iTime)
							{
								iTempWeight *= 10;
							}
						}
						else if((kPlayer.getNumCities()+iSettlersOnMap) < 6)
						{
							int iTime = 100;
							iTime *= GC.getGame().getGameSpeedInfo().getConstructPercent();
							iTime /= 100;
						...
						else if((kPlayer.getNumCities()+iSettlersOnMap) < 7)
						{
							int iTime = 150;
							iTime *= GC.getGame().getGameSpeedInfo().getConstructPercent();
							iTime /= 100;
						...
						else if((kPlayer.getNumCities()+iSettlersOnMap) < 8)
						{
							int iTime = 200;
							iTime *= GC.getGame().getGameSpeedInfo().getConstructPercent();
							iTime /= 100;
...
#ifdef AMB_AI_PRODUCTION_UNIT_DEFENSLESS
/*				int iTimeDefenseless = 45;
				iTimeDefenseless *= GC.getGame().getGameSpeedInfo().getConstructPercent();
				iTimeDefenseless /= 100;
				if(GC.getGame().getGameTurn()>iTimeDefenseless && kPlayer.GetMilitaryAI()->GetLandDefenseState() > DEFENSE_STATE_NEEDED)
				{
					if(pkUnitEntry)
					{
						DomainTypes eDomain = (DomainTypes) pkUnitEntry->GetDomainType();
						if((eDomain == DOMAIN_LAND || eDomain == DOMAIN_AIR) && pkUnitEntry->IsMilitaryProduction())
						{
							iTempWeight *= 2;
						}
					}
				}
				if(GC.getGame().getGameTurn()>iTimeDefenseless && kPlayer.GetMilitaryAI()->GetNavalDefenseState() > DEFENSE_STATE_NEEDED)
				{
					if(pkUnitEntry)
					{
						DomainTypes eDomain = (DomainTypes) pkUnitEntry->GetDomainType();
						if(eDomain == DOMAIN_SEA && pkUnitEntry->IsMilitaryProduction())
						{
							iTempWeight *= 2;
						}
					}
				}*/
				//Let's defend against humans !
				int iTimeDefenseless = 40;
				iTimeDefenseless *= GC.getGame().getGameSpeedInfo().getConstructPercent();
				iTimeDefenseless /= 100;
				bool bSneakyHumanNeighbour=false;
				PlayerTypes eHumanPlayer = NO_PLAYER;
				//only major civ
				if(!kPlayer.isMinorCiv())
				{
					for(int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
					{
						...
						eHumanPlayer = (PlayerTypes) iPlayerLoop;

						//need neighbouur
						if(kPlayer.GetProximityToPlayer((PlayerTypes) iPlayerLoop) == PLAYER_PROXIMITY_NEIGHBORS)
						{
							bSneakyHumanNeighbour = true;
							break;
						}
				...
				//We want more units
				if(eHumanPlayer != NO_PLAYER)
				{				
					if(bSneakyHumanNeighbour && GC.getGame().getGameTurn()>iTimeDefenseless && (kPlayer.GetMilitaryAI()->GetLandDefenseState() > DEFENSE_STATE_NEEDED || kPlayer.GetDiplomacyAI()->GetWarState(eHumanPlayer) < WAR_STATE_CALM))
					{
						if(pkUnitEntry)
						{
							DomainTypes eDomain = (DomainTypes) pkUnitEntry->GetDomainType();
							if((eDomain == DOMAIN_LAND || eDomain == DOMAIN_AIR) && pkUnitEntry->IsMilitaryProduction())
							{
								AMB_LOG("%s has a Sneaky human neighbour boosting unit likelihood ! x2", kPlayer.getName());
								iTempWeight *= 2;
							}
						}
					}
...
#ifdef AMB_CITYSTRATEGY_FIX_CHOOSE_PRODUCTION_ACCURATE_SEA_SANITY_CHECK_ONLY_MAJOR
...
#ifdef AMB_LESS_UNIT_CS
				if(kPlayer.isMinorCiv() && pkUnitEntry->GetCombat() > 0 && kPlayer.GetNumUnitsWithDomain((DomainTypes)pkUnitEntry->GetDomainType(),true)>=kPlayer.GetMilitaryAI()->GetMandatoryReserveSize())
				{
					iTempWeight = 10;			
				}
#endif
...
#ifdef AMB_AI_PRODUCTION_TWEAKS
				else if(strStrategyName == "AICITYSTRATEGY_SECOND_CULTURE_BUILDING")
					bStrategyShouldBeActive = CityStrategyAIHelpers::IsTestCityStrategy_SecondCultureBuilding(GetCity());
				else if(strStrategyName == "AICITYSTRATEGY_THIRD_CULTURE_BUILDING")
					bStrategyShouldBeActive = CityStrategyAIHelpers::IsTestCityStrategy_ThirdCultureBuilding(GetCity());
				else if(strStrategyName == "AICITYSTRATEGY_FOURTH_CULTURE_BUILDING")
					bStrategyShouldBeActive = CityStrategyAIHelpers::IsTestCityStrategy_FourthCultureBuilding(GetCity());
				else if(strStrategyName == "AICITYSTRATEGY_FIFTH_CULTURE_BUILDING")
					bStrategyShouldBeActive = CityStrategyAIHelpers::IsTestCityStrategy_FifthCultureBuilding(GetCity());
				else if(strStrategyName == "AICITYSTRATEGY_SECOND_SCIENCE_BUILDING")
					bStrategyShouldBeActive = CityStrategyAIHelpers::IsTestCityStrategy_SecondScienceBuilding(GetCity());
				else if(strStrategyName == "AICITYSTRATEGY_THIRD_SCIENCE_BUILDING")
					bStrategyShouldBeActive = CityStrategyAIHelpers::IsTestCityStrategy_ThirdScienceBuilding(GetCity());
				else if(strStrategyName == "AICITYSTRATEGY_FOURTH_SCIENCE_BUILDING")
#endif
					bStrategyShouldBeActive = CityStrategyAIHelpers::IsTestCityStrategy_FourthScienceBuilding(GetCity());
...
#ifdef AMB_AUIFIX_WORKER_DISBAND_ADJUSTMENT
	/*if(iNumBuilders >= 11)
	{
		return true;
	}*/
		...
		if(!kPlayer.isMinorCiv())
		{
...
#ifdef AMB_AI_NUM_CITIES_TWEAKS
			if(GC.getGame().getGameTurn()*100 < (18 * GC.getGame().getGameSpeedInfo().getResearchPercent()))
			{
				return false;
			}

			if((iCitiesPlusSettlers) < 4)
#else
...
#endif
			{
				...
				if((iCitiesPlusSettlers == 1 && (iGameTurn * 4) > iWeightThreshold) ||
					(iCitiesPlusSettlers == 2 && (iGameTurn * 3) > iWeightThreshold) || 
					(iCitiesPlusSettlers == 3 && iGameTurn * 2 > iWeightThreshold) ||
					(iCitiesPlusSettlers == 4 && iGameTurn > iWeightThreshold)) 
#else
				if((iCitiesPlusSettlers == 1 && (iGameTurn * 4) > iWeightThreshold) ||
...
{
#ifdef AMB_AI_PRODUCTION_TWEAKS
	if(pCity->GetJONSCulturePerTurnFromBuildings()-pCity->GetCultureFromWonders() <= 1)
	{
		return true;
	}
	if(pCity->isCapital() && pCity->GetJONSCulturePerTurnFromBuildings()-pCity->GetCultureFromWonders() <= 3)
	{
		return true;
	}
#else
...
#ifdef AMB_AI_PRODUCTION_TWEAKS
bool CityStrategyAIHelpers::IsTestCityStrategy_SecondCultureBuilding(CvCity* pCity)
{
	if(pCity->GetPlayer()->GetCurrentEra()>= 2 && pCity->getPopulation() >= 8)
	{
		if(pCity->GetJONSCulturePerTurnFromBuildings()-pCity->GetCultureFromWonders() <= 3)
		{
			return true;
		}

		if(pCity->isCapital() && pCity->GetJONSCulturePerTurnFromBuildings()-pCity->GetCultureFromWonders() <= 5)
		{
			return true;
		}
	}
...
bool CityStrategyAIHelpers::IsTestCityStrategy_ThirdCultureBuilding(CvCity* pCity)
{
	if(pCity->GetPlayer()->GetCurrentEra()>= 3 && pCity->getPopulation() >= 11)
	{
		if(pCity->GetJONSCulturePerTurnFromBuildings()-pCity->GetCultureFromWonders() <= 5)
		{
			return true;
		}

		if(pCity->isCapital() && pCity->GetJONSCulturePerTurnFromBuildings()-pCity->GetCultureFromWonders() <= 7)
		{
			return true;
		}
	}
...
bool CityStrategyAIHelpers::IsTestCityStrategy_FourthCultureBuilding(CvCity* pCity)
{
	if(pCity->GetPlayer()->GetCurrentEra()>= 4 && pCity->getPopulation() >= 15)
	{
		if(pCity->GetJONSCulturePerTurnFromBuildings()-pCity->GetCultureFromWonders() <= 7)
		{
			return true;
		}

		if(pCity->isCapital() && pCity->GetJONSCulturePerTurnFromBuildings()-pCity->GetCultureFromWonders() <= 9)
		{
			return true;
		}
	}
...
bool CityStrategyAIHelpers::IsTestCityStrategy_FifthCultureBuilding(CvCity* pCity)
{
	if(pCity->GetPlayer()->GetCurrentEra()>= 5 && pCity->getPopulation() >= 20)
	{
		if(pCity->GetJONSCulturePerTurnFromBuildings()-pCity->GetCultureFromWonders() <= 9)
		{
			return true;
		}

		if(pCity->isCapital() && pCity->GetJONSCulturePerTurnFromBuildings()-pCity->GetCultureFromWonders() <=11 )
		{
			return true;
		}
	}
...
bool CityStrategyAIHelpers::IsTestCityStrategy_SecondScienceBuilding(CvCity* pCity)
{
	if(pCity->GetPlayer()->GetCurrentEra()>= 2 && pCity->getPopulation() >= 7 && pCity->getYieldRateModifier(YIELD_SCIENCE)<=32)
	{
		return true;
	}
...
bool CityStrategyAIHelpers::IsTestCityStrategy_ThirdScienceBuilding(CvCity* pCity)
{
	if(pCity->GetPlayer()->GetCurrentEra()>= 4 && pCity->getPopulation() >= 13 && pCity->GetYieldPerPopTimes100(YIELD_SCIENCE)<100)
	{
		return true;
	}
...
bool CityStrategyAIHelpers::IsTestCityStrategy_FourthScienceBuilding(CvCity* pCity)
{
	if(pCity->GetPlayer()->GetCurrentEra()>= 5 && pCity->getPopulation() >= 15 && pCity->getYieldRateModifier(YIELD_SCIENCE)<=80)
	{
		return true;
	}
...
{
#ifdef AMB_AI_PRODUCTION_TWEAKS
	if(pCity->getPopulation() >= 5 && pCity->GetJONSCulturePerTurnFromBuildings() <= 1)
	{
		return true;
	}
#else
...
{
	if(pCity->getPopulation() >= 5 && pCity->GetYieldPerPopTimes100(YIELD_SCIENCE) == 0)
	{
		return true;
	}
...
#ifdef AMB_RELIGIONAI_TWEAKS
	if(iReligionFlavor > 5 || pCity->isCapital())
#else
...
{
#ifdef AMB_DIFFICULTY_ID_ADJUSTMENT
	if(pCity->getPopulation() >= 5 && GC.getGame().getHandicapInfo().GetID() > 3)
#else
...
#ifdef AMB_MOD_H
	if((GC.getGame().getGameTurn()*GC.getGame().getGameSpeedInfo().getConstructPercent())/100 < 40)
	{
		return false;
	}
#endif
