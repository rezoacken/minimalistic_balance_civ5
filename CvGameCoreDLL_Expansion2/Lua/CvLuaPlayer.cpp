...
int CvLuaPlayer::lGetOpinionTable(lua_State* L)
{
...
#ifdef AMB_DONT_HIDE_LAND_DISPUTE
	// land dispute
	...
	if(pDiploAI->HasCasusBelliAgainst(eWithPlayer))
	{
		Opinion kOpinion;
		kOpinion.m_iValue = 0;
		kOpinion.m_str = Localization::Lookup("TXT_KEY_DIPLO_CASUS_BELLI_THEM");
		aOpinions.push_back(kOpinion);
	}
	if(GET_PLAYER(eWithPlayer).GetDiplomacyAI()->HasCasusBelliAgainst(pkPlayer->GetID()))
	{
		Opinion kOpinion;
		kOpinion.m_iValue = -1;
		kOpinion.m_str = Localization::Lookup("TXT_KEY_DIPLO_CASUS_BELLI_US");
		aOpinions.push_back(kOpinion);
	}
#endif
	...
	if (iValue != 0)
	{
		...
		CvString str;	
		if (pDiploAI->GetRecklessExpanderLevel(eWithPlayer) == RECKLESS_EXPANDER_CRITICAL)
		{
			str = Localization::Lookup("TXT_KEY_DIPLO_RECKLESS_EXPANDER_CRITICAL").toUTF8();
		}
		else if (pDiploAI->GetRecklessExpanderLevel(eWithPlayer) == RECKLESS_EXPANDER_SEVERE)
		{
			str = Localization::Lookup("TXT_KEY_DIPLO_RECKLESS_EXPANDER_SEVERE").toUTF8();
		}
		else if (pDiploAI->GetRecklessExpanderLevel(eWithPlayer) == RECKLESS_EXPANDER_MAJOR)
		{
			str = Localization::Lookup("TXT_KEY_DIPLO_RECKLESS_EXPANDER_MAJOR").toUTF8();
		}
		else 
		{
			str = Localization::Lookup("TXT_KEY_DIPLO_RECKLESS_EXPANDER_MINOR").toUTF8();
		}
		kOpinion.m_str = str;
		aOpinions.push_back(kOpinion);
#else
