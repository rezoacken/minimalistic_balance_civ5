...
#ifdef AMB_TECH_PENALTY_OVERHAUL
		//Is this a human
		if(m_pPlayer->isHuman())
		{
			iMod = (int)((double) iMod * pow((double)m_pPlayer->GetMaxEffectiveCities(/*bIncludePuppets*/ true),0.75));
			//AMB_LOG("Human");
			//AMB_LOG("PlayerTechs get research Research mod cost is %i for %s",iMod,m_pPlayer->getName());
		}
		//else apply the AI exponent
		else
		{
			iMod = (int)((double) iMod * pow((double)m_pPlayer->GetMaxEffectiveCities(/*bIncludePuppets*/ true),(double)GC.getGame().getHandicapInfo().getAINumCitiesExponent()/100.0));
			//AMB_LOG("PlayerTechs get research Research mod cost is %i for %s",iMod,m_pPlayer->getName());
		}
#else
...
#ifdef AMB_TECH_PENALTY_OVERHAUL
		//Is there a human in the team
		if(GET_PLAYER(ePlayer).isHuman())
		{
			iNumCitiesMod = (int)((double) iNumCitiesMod * pow((double)GET_PLAYER(ePlayer).GetMaxEffectiveCities(/*bIncludePuppets*/ true),0.75));
		}
		//else apply the AI exponent
		else
		{
			iNumCitiesMod = (int)((double) iNumCitiesMod * pow((double)GET_PLAYER(ePlayer).GetMaxEffectiveCities(/*bIncludePuppets*/ true),GC.getGame().getHandicapInfo().getAINumCitiesExponent()/100.0));
		}
#else
