...
#ifdef AMB_POLICY_GREATWORKTOURISMCHANGE
	iTourismPerWork += GET_PLAYER(eOwner).GetGreatWorkYieldChange(YIELD_TOURISM);
//AMB_LOG("Tourism read %i", GET_PLAYER(eOwner).GetGreatWorkYieldChange(YIELD_TOURISM));
#endif
...
#ifdef AMB_AI_GS_CULTURE_POPULAR_FAMILIAR_CIV_MOD
///Mod: How many civs do we have popular culture over ?
int CvPlayerCulture::GetNumCivsPopularOn() const
{
	int iRtnValue = 0;
	...
	for (int iLoopPlayer = 0; iLoopPlayer < MAX_MAJOR_CIVS; iLoopPlayer++)
	{
		...
		if (iLoopPlayer != m_pPlayer->GetID() && kPlayer.isAlive() && !kPlayer.isMinorCiv())
		{
			if (GetInfluenceLevel(kPlayer.GetID()) >= INFLUENCE_LEVEL_POPULAR)
			{
				iRtnValue++;
			}
		}
...
///Mod: How many civs do we have Familiar culture over ?
int CvPlayerCulture::GetNumCivsFamiliarOn() const
{
	int iRtnValue = 0;
	...
	for (int iLoopPlayer = 0; iLoopPlayer < MAX_MAJOR_CIVS; iLoopPlayer++)
	{
		...
		if (iLoopPlayer != m_pPlayer->GetID() && kPlayer.isAlive() && !kPlayer.isMinorCiv())
		{
			if (GetInfluenceLevel(kPlayer.GetID()) >= INFLUENCE_LEVEL_FAMILIAR)
			{
				iRtnValue++;
			}
		}
...
#ifdef AMB_POLICY_GREATWORKTOURISMCHANGE
	int iBase = GetNumGreatWorks();
	iBase *= GC.getBASE_TOURISM_PER_GREAT_WORK() + GET_PLAYER(m_pCity->getOwner()).GetGreatWorkYieldChange(YIELD_TOURISM);
	//AMB_LOG("Total tourism GW is %i", iBase);
...
#if defined(AMB_POLICY_SECONDARYRELIGIONFOLLOWERBELIEFS_HERMITAGE)
					BeliefTypes eSecondaryFollowerOne = m_pCity->GetCityReligions()->GetSecondaryReligionFollowerBelief(false);
					BeliefTypes eSecondaryFollowerTwo = m_pCity->GetCityReligions()->GetSecondaryReligionFollowerBelief(true);
					if(eSecondaryFollowerOne!=NO_BELIEF)
					{
						iBase += GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->GetBuildingClassTourism(eBuildingClass);
						if(eSecondaryFollowerTwo!=NO_BELIEF)
						{
							iBase += GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->GetBuildingClassTourism(eBuildingClass);
						}
					}
#endif
				}
...
#ifdef AMB_AI_DIFFICULTY_TOURISM_BONUS
	if(!kCityPlayer.isHuman())
	{
		iMultiplier += GC.getGame().getHandicapInfo().getAITourismAdditiveModifier();
	}
#endif
...
CvString CvCityCulture::GetTourismTooltip()
{
	...
	int iGWTourism = GetNumGreatWorks() * (GC.getBASE_TOURISM_PER_GREAT_WORK() + GET_PLAYER(m_pCity->getOwner()).GetGreatWorkYieldChange(YIELD_TOURISM));
#else
		...
		for(int jJ = 0; jJ < GC.getNumBuildingClassInfos(); jJ++)
		{
			...
			if(eBuilding != NO_BUILDING)
			{
				if(m_pCity->GetCityBuildings()->GetNumBuilding(eBuilding) > 0)
				{
					...
					if(eSecondaryFollowerOne!=NO_BELIEF)
					{
						iReligiousArtTourism += GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerOne)->GetBuildingClassTourism(eBuildingClass);
						if(eSecondaryFollowerTwo!=NO_BELIEF)
						{
							iReligiousArtTourism += GC.GetGameBeliefs()->GetEntry(eSecondaryFollowerTwo)->GetBuildingClassTourism(eBuildingClass);
						}
					}
#endif
				}
	...
	for(int jJ = 0; jJ < GC.getNumBuildingClassInfos(); jJ++)
	{
...
#if defined(AMB_POLICY_BUILDINGCHANGETOURISMCHANGES)
				iTourismFromBuildings += GET_PLAYER(m_pCity->getOwner()).GetPlayerPolicies()->GetBuildingClassYieldChange(eBuildingClass,YIELD_TOURISM);
#endif
			}
		}
	}
		...
		szRtnValue += GetLocalizedText("TXT_KEY_CO_CITY_TOURISM_BUILDINGCLASS", iTourismFromBuildings);
#endif
	int iBuildingMod = 0;
	for(int iBuildingClassLoop = 0; iBuildingClassLoop < GC.getNumBuildingClassInfos(); iBuildingClassLoop++)
	{
