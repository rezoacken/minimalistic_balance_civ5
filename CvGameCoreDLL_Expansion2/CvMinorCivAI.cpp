...
void CvMinorCivAI::DoThreateningWarmongerKilled(PlayerTypes eKillingPlayer, int iX, int iY)
{
	CvAssertMsg(eKillingPlayer >= 0, "eMajor is expected to be non-negative (invalid Index)");
	CvAssertMsg(eKillingPlayer < MAX_MAJOR_CIVS, "eMajor is expected to be within maximum bounds (invalid Index)");

	ChangeFriendshipWithMajor(eKillingPlayer, 12);
	ChangeAngerFreeIntrusionCounter(eKillingPlayer, 5);
	...
	for(int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
	{
		...
		if(IsHasMetPlayer(ePlayer))
		{
			...
			if(GetQuestCountdownForPlayer(ePlayer) > 0)
			{
				...
				int iSpeed = 1 + GET_PLAYER(ePlayer).GetPlayerPolicies()->GetNumericModifier(POLICYMOD_FASTER_PERSONAL_QUEST);
				ChangeQuestCountdownForPlayer(ePlayer, -iSpeed);
//AMB_LOG("Player %s has countdown equal to %i with %s, lost %i", GET_PLAYER(ePlayer).getName(),GetQuestCountdownForPlayer(ePlayer), m_pPlayer->getName(), iSpeed);
#else
...
void CvMinorCivAI::DoTestStartPersonalQuest(PlayerTypes ePlayer)
{
	...
	int iMaxPersonalQuest = GetMaxActivePersonalQuestsForPlayer();
	iMaxPersonalQuest += GET_PLAYER(ePlayer).GetPlayerPolicies()->GetNumericModifier(POLICYMOD_ADDITIONAL_PERSONAL_QUEST);
	if(GetNumActivePersonalQuestsForPlayer(ePlayer) >= iMaxPersonalQuest)
	{
		return;
	}
#else
...
#if defined(AMB_POLICY_ADDITIONALPERSONALQUEST)
		int iMaxPersonalQuest = GetMaxActivePersonalQuestsForPlayer();
		iMaxPersonalQuest += GET_PLAYER(eMajor).GetPlayerPolicies()->GetNumericModifier(POLICYMOD_ADDITIONAL_PERSONAL_QUEST);
		if(GetNumActivePersonalQuestsForPlayer(eMajor) >= iMaxPersonalQuest)
		{
			return false;
		}
#else
...
#if defined(AMB_POLICY_FIXEDSCHOLASTICISM) 
	EraTypes eEra = GET_PLAYER(m_pPlayer->GetID()).GetCurrentEra();
	int iResult = 0;
	switch(eEra)
	{
	case 0:
...
int CvMinorCivAI::GetCurrentCultureFlatBonus(PlayerTypes ePlayer)
{
	...
	iModifier += GET_PLAYER(ePlayer).GetPlayerPolicies()->GetNumericModifier(POLICYMOD_BONUS_MINOR_YIELD);
#endif
	if(iModifier > 0)
	{
...
int CvMinorCivAI::CalculateBullyMetric(PlayerTypes eBullyPlayer, bool bForUnit, CvString* sTooltipSink)
{
	...
	// +0 ~ +125
	// **************************
#ifdef AMB_HARDER_BULLY
	int iComparisonRadius = 5;
#else
...
#ifdef AMB_HARDER_BULLY
	iMinorLocalPower += 4 + pMinorCapital->GetPower();
#else
...
#ifdef AMB_HARDER_BULLY
							else if(pLoopUnit->getOwner() == GetPlayer()->GetID() && !pLoopUnit->IsRecon())
#else
