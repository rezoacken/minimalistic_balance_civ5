...
void CvBuilderTaskingAI::ConnectCitiesToCapital(CvCity* pPlayerCapital, CvCity* pTargetCity, RouteTypes eRoute)
{
	...
	if(bIndustrialRoute)
	{
		if(iProfit >= 0)
		{
#ifdef AMB_BUILDER_TWEAKS
			//less running around for railroads, value it as a normal road
			sValue = min((iGoldForRoute * 100) / (iMaintenancePerTile + 1) / iPlotsNeeded, MAX_SHORT);
		}
#else
			sValue = MAX_SHORT;
		}
	...
	{
		sValue = min(GC.getMINOR_CIV_ROUTE_QUEST_WEIGHT() / iPlotsNeeded, MAX_SHORT); //1000
	}
	else // normal route
	{
...
#ifndef AMB_BUILDER_TWEAKS //Why does it matter ?
	...
	for(uint ui = 0; ui < NUM_YIELD_TYPES; ui++)
	{
		...
		for(int iFlavorLoop = 0; iFlavorLoop < GC.getNumFlavorTypes(); iFlavorLoop++)
		{
			...
			case YIELD_FOOD:
				...
				{
					iYieldDifferenceWeight += int(iDeltaYield * log((double)MAX(1, pFlavorManager->GetPersonalityIndividualFlavor((FlavorTypes)iFlavorLoop))) * GC.getBUILDER_TASKING_PLOT_EVAL_MULTIPLIER_FOOD() + 0.5);
				}
				break;
			case YIELD_PRODUCTION:
				if (GC.getFlavorTypes((FlavorTypes)iFlavorLoop) == "FLAVOR_PRODUCTION")
				{
					iYieldDifferenceWeight += int(iDeltaYield * log((double)MAX(1, pFlavorManager->GetPersonalityIndividualFlavor((FlavorTypes)iFlavorLoop))) * GC.getBUILDER_TASKING_PLOT_EVAL_MULTIPLIER_PRODUCTION() + 0.5);
				}
				break;
			case YIELD_GOLD:
				if (GC.getFlavorTypes((FlavorTypes)iFlavorLoop) == "FLAVOR_GOLD")
				{
					iYieldDifferenceWeight += int(iDeltaYield * log((double)MAX(1, pFlavorManager->GetPersonalityIndividualFlavor((FlavorTypes)iFlavorLoop))) * GC.getBUILDER_TASKING_PLOT_EVAL_MULTIPLIER_GOLD() + 0.5);
				}
				break;
			case YIELD_SCIENCE:
				if (GC.getFlavorTypes((FlavorTypes)iFlavorLoop) == "FLAVOR_SCIENCE")
				{
					iYieldDifferenceWeight += int(iDeltaYield * log((double)MAX(1, pFlavorManager->GetPersonalityIndividualFlavor((FlavorTypes)iFlavorLoop))) * GC.getBUILDER_TASKING_PLOT_EVAL_MULTIPLIER_SCIENCE() + 0.5);
				}
				break;
			case YIELD_CULTURE:
...
#ifdef AMB_BUILDER_TWEAKS
	return 10000 / ((int) pow((double) iBuildTime,(double) 1.7));
#else
...
#ifdef AMB_BUILDER_TWEAKS
		//Improvements probably shouldn't be influenced too much by personality value between 4 and 6 now
		int iPersonalityFlavorValue = max(4,min(6,m_pPlayer->GetFlavorManager()->GetPersonalityIndividualFlavor((FlavorTypes)i)));
#else
...
#endif // AUI_WORKER_SCORE_PLOT_CHOP
{
	...
	if (!pCity->GetPlayer()->isHuman())
	{
		...
		{
			CityAIFocusTypes eFocus = pCity->GetCityCitizens()->GetFocusType();
		...
		if (pCity->GetPlayer()->GetExcessHappiness() <= 0 && !pCity->IsIgnoreCityForHappiness())
		{
...
#ifdef AMB_BETTER_PLOT_BUILD_VALUE_USE_CITIZEN
		switch(ui)
		{
		case YIELD_FOOD:
			iYieldDelta*=GC.getAI_CITIZEN_VALUE_FOOD();
			iYieldDelta/=5;
			break;
		case YIELD_GOLD:
			iYieldDelta*=GC.getAI_CITIZEN_VALUE_GOLD();
			iYieldDelta/=5;
			break;
		case YIELD_SCIENCE:
			iYieldDelta*=GC.getAI_CITIZEN_VALUE_SCIENCE();
			iYieldDelta/=5;
			break;
		case YIELD_PRODUCTION:
			iYieldDelta*=GC.getAI_CITIZEN_VALUE_PRODUCTION();
			iYieldDelta/=5;
			break;
		case YIELD_CULTURE:
			iYieldDelta*=GC.getAI_CITIZEN_VALUE_CULTURE();
			iYieldDelta/=5;
			break;
		case YIELD_FAITH:
			iYieldDelta*=GC.getAI_CITIZEN_VALUE_FAITH();
			iYieldDelta/=5;
			break;
		}
#endif
...
#ifdef AMB_BUILDER_TWEAKS
	if(m_pTargetPlot->isBeingWorked())
	{
		iScore *=3;
		iScore /=2;
	}
