...
#endif
	//modAcken if we have captured some capital increase the priority
#if defined(AMB_AI_GS_CONQUEST_CAPITAL_CONQUERED_MOD)
	//Should also be proportional to number of civs.
	//ApolloWeight = (TotalCiv - CapturedCaps) * Weight
	int iApollo = GC.getAI_GS_SS_HAS_APOLLO_PROGRAM();
	int iCapitalWeight = iApollo / (max(1,GC.getGame().countMajorCivsAlive()-1));
	for(int iMajorLoop = 0; iMajorLoop < MAX_MAJOR_CIVS; iMajorLoop++)
	{
			
			PlayerTypes ePlayer = (PlayerTypes) iMajorLoop;
			if(GET_PLAYER(ePlayer).GetCapitalConqueror()==GetPlayer()->GetID() && iMajorLoop != GetPlayer()->GetID() && !GET_PLAYER(ePlayer).isMinorCiv())
			{
					iPriority += iCapitalWeight;
			}
	}
	if(GC.getGame().countMajorCivsEverAlive()==2)
	{
		iPriority *= 2;
	}
#endif
...
#if defined(AMB_AI_GS_CULTURE_POPULAR_FAMILIAR_CIV_MOD)
//#if defined(AMB_AI_GS_CULTURE_FAMILIAR_CIV_MOD)
//	int iNumFamiliar = m_pPlayer->GetCulture()->GetNumCivsFamiliarOn();
//	iPriority += iNumFamiliar * /*15*/ GC.getAI_GS_CULTURE_FAMILIAR_CIV_MOD();
//#endif
	
	//modAcken for familiar and popular levels
	//New: The bonus should be relative to the number of civs left !
	//(Numciv.isalive - 1)*Influential = Appollo priority
	//Influential = Appollo / (Numciv - 1)
	
	int iApollo = GC.getAI_GS_SS_HAS_APOLLO_PROGRAM();
	int iAlive = max(1,GC.getGame().countMajorCivsAlive()-1);
	int iInfluential = iApollo / iAlive;
	int iPopular = iInfluential * 20;
	iPopular /= 100;
	int iFamiliar = iInfluential * 40;
	iFamiliar = iPopular /= 100;
	iInfluential -= (iPopular+iFamiliar);
	int iNumPopular = m_pPlayer->GetCulture()->GetNumCivsPopularOn();
	int iNumInfluential = m_pPlayer->GetCulture()->GetNumCivsInfluentialOn();
	int iNumFamiliar = m_pPlayer->GetCulture()->GetNumCivsFamiliarOn();
	iPriority += iNumPopular * iPopular;
	iPriority += iNumFamiliar * iFamiliar;
	iPriority += iNumInfluential * iInfluential;
#else
	// for every civ we are Influential over increase this
	int iNumInfluential = m_pPlayer->GetCulture()->GetNumCivsInfluentialOn();
	iPriority += iNumInfluential * /*30*/ GC.getAI_GS_CULTURE_INFLUENTIAL_CIV_MOD();
#endif
