...
int CvGameTrade::GetDomainModifierTimes100 (DomainTypes eDomain)
{
	if (eDomain == DOMAIN_SEA)
	{
#ifdef AMB_WEAKER_SEA_TRADE_ROUTES
		return 50;
#else
		return 100;
#endif
...
#ifdef AMB_POLICY_YIELDCHANGEPERSEAINTERNATIONALTRADEROUTE
				if(kTradeConnection.m_eDomain == DOMAIN_SEA)
				{
					int iPolicyBoost = GET_PLAYER(kTradeConnection.m_eOriginOwner).GetPlayerPolicies()->GetYieldChangePerSeaInternationalTradeRoute(YIELD_SCIENCE);
					iAdjustedTechDifference += iPolicyBoost;
				}
#endif
...
int CvPlayerTrade::GetTradeConnectionValueTimes100 (const TradeConnection& kTradeConnection, YieldTypes eYield, bool bAsOriginPlayer)
{
	...
	if (bAsOriginPlayer)
	{
		if (pTrade->IsConnectionInternational(kTradeConnection))
		{
			...
			case YIELD_TOURISM:
	#ifdef AMB_API_UNIFIED_YIELDS_TOURISM_NO_TRADE_ROUTE_TOURISM
			break;
	#endif
#endif
			case YIELD_SCIENCE:
...
//Returns the number of city state trade routes connected to a city //NoteAcken doesn't seem to be for a city only but globally
...
uint CvPlayerTrade::GetNumTradeRoutesPossible (void)
{
	...
	//modAcken Additional policies giving extra trade routes
	iNumRoutes += m_pPlayer->GetPlayerPolicies()->GetNumberExtraTradeRoutes();
	//endmod
#endif
...
//----------------------------------------------------------------------------------
#ifdef AMB_POLICY_YIELDCHANGEPERSEAINTERNATIONALTRADEROUTE
int CvPlayerTrade::GetNumTradeInternationalRoutes (bool bSea)
{
	CvGameTrade* pTrade = GC.getGame().GetGameTrade();
	int iNumConnections = 0;
	for (uint ui = 0; ui < pTrade->m_aTradeConnections.size(); ui++)
	{
		TradeConnection* pConnection = &(pTrade->m_aTradeConnections[ui]);
		if(!bSea || pConnection->m_eDomain == DOMAIN_SEA)
		{
			if (pConnection->m_eOriginOwner == m_pPlayer->GetID())
			{
				if(!GET_PLAYER(pConnection->m_eDestOwner).isMinorCiv())
...
int CvTradeAI::ScoreInternationalTR (const TradeConnection& kTradeConnection)
{
...
#ifdef AMB_AI_TRADE_REDUCED_TARGET_GOLD_NEGATIVE_EFFECT
	int iGoldDelta = iGoldAmount - iOtherGoldAmount/4;
#else
	int iGoldDelta = iGoldAmount - iOtherGoldAmount; //NoteAcken This is stupid, a trade route giving 1vs0 will be valued higher than 20vs20.
#endif
...
#ifdef AMB_AI_TRADE_REDUCED_TARGET_GOLD_NEGATIVE_EFFECT
	int iTechDelta = iTechDifferenceP1fromP2 - iTechDifferenceP2fromP1/4;
#else	
	...
	iScore += iTechDelta * 3; // 3 science = 1 gold //NoteAcken No this means 1 tech = 3 gold

#ifdef AMB_AI_TRADE_REDUCED_TARGET_GOLD_NEGATIVE_EFFECT
	iScore += iReligionDelta;
#else	
	iScore += iReligionDelta * 2; // 2 religion = 1 gold //NoteAcken No also
#endif
...
//increase score if we have Patronage
#if defined(AMB_POLICY_YIELDCHANGEPERMINORTRADEROUTE)
	if ((m_pPlayer->GetPlayerPolicies()->GetYieldChangePerMinorTradeRoute(YIELD_CULTURE) > 0 || m_pPlayer->GetPlayerPolicies()->GetYieldChangePerMinorTradeRoute(YIELD_SCIENCE) > 0) && GET_PLAYER(kTradeConnection.m_eDestOwner).isMinorCiv())
	{
		iScore += m_pPlayer->GetPlayerPolicies()->GetYieldChangePerMinorTradeRoute(YIELD_CULTURE)*2;
		iScore += m_pPlayer->GetPlayerPolicies()->GetYieldChangePerMinorTradeRoute(YIELD_SCIENCE)*3;
	}
#endif
#if defined(AMB_POLICY_YIELDCHANGEPERSEAINTERNATIONALTRADEROUTE)
	if ((m_pPlayer->GetPlayerPolicies()->GetYieldChangePerSeaInternationalTradeRoute(YIELD_CULTURE) > 0 || m_pPlayer->GetPlayerPolicies()->GetYieldChangePerSeaInternationalTradeRoute(YIELD_SCIENCE) > 0) && !GET_PLAYER(kTradeConnection.m_eDestOwner).isMinorCiv())
	{
		if(kTradeConnection.m_eDomain == DOMAIN_SEA)
		{

			iScore += m_pPlayer->GetPlayerPolicies()->GetYieldChangePerSeaInternationalTradeRoute(YIELD_CULTURE)*2;
			iScore += m_pPlayer->GetPlayerPolicies()->GetYieldChangePerSeaInternationalTradeRoute(YIELD_SCIENCE)*3;
...
#ifdef AMB_BETTER_INTERNAL_ROUTE_VALUATION
	int iFoodDeficit = 10;
	iFoodDeficit -= pSmallestCity->foodDifference();
	iDistanceScore *= iFoodDeficit;
	iDistanceScore /= 2;
#endif
...
{	
	...
	if (m_pPlayer->GetHappiness() >= 0)
	{
		...
		{
#ifdef AMB_BETTER_INTERNAL_ROUTE_VALUATION
			//Based on food rather than size
			int iCityFood = pCity->foodDifference();
			if (iCityFood < iSmallestCitySize)
			{
				pSmallestCity = pCity;
				iSmallestCitySize = iCityFood;
			}
#else
