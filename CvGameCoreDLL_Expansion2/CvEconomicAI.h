...
public:
	...
	int GetMoneyForRushs() const
	{
		return m_iMoneyForRushs;
	};
	int GetMoneyForPlots() const
	{
		return m_iMoneyForPlots;
	};
	int GetMoneyForMinorGifts() const
	{
		return m_iMoneyForMinorGifts;
	};
	int GetMoneyForUnitUpgrades() const
	{
		return m_iMoneyForUnitUpgrades;
	};
	//int GetPreviousTreasury() const
	//{
	//	return m_iPreviousTreasury;
	//};
	void ChangeMoneyForRushs(int iValue)
	{
		m_iMoneyForRushs = max(0,m_iMoneyForRushs+iValue);
	};
	void ChangeMoneyForPlots(int iValue)
	{
		m_iMoneyForPlots = max(0,m_iMoneyForPlots+iValue);
	};
	void ChangeMoneyForMinorGifts(int iValue)
	{
		m_iMoneyForMinorGifts = max(0,m_iMoneyForMinorGifts+iValue);
	};
	void ChangeMoneyForUnitUpgrades(int iValue)
	{
		m_iMoneyForUnitUpgrades = max(0,m_iMoneyForUnitUpgrades+iValue);
	};
	void SetMoneyForRushs(int iValue)
	{
		m_iMoneyForRushs = max(0,iValue);
	};
	void SetMoneyForPlots(int iValue)
	{
		m_iMoneyForPlots = max(0,iValue);
	};
	void SetMoneyForMinorGifts(int iValue)
	{
		m_iMoneyForMinorGifts = max(0,iValue);
	};
	void SetMoneyForUnitUpgrades(int iValue)
	{
		m_iMoneyForUnitUpgrades = max(0,iValue);
	};
	//void SetPreviousTreasury (int iValue) const
	//{
	//	m_iPreviousTreasury = iValue;
	//};
#endif
...
#ifdef AMB_AI_TREASURY_MANAGEMENT
	int m_iMoneyForRushs;
	int m_iMoneyForPlots;
	int m_iMoneyForMinorGifts;
	int m_iMoneyForUnitUpgrades;
	int m_iPreviousTreasury;
#endif
...
bool IsTestStrategy_IsReligiousCiv(CvPlayer* pPlayer);
/*
bool IsTestStrategy_HasMerchantConfederacy(CvPlayer* pPlayer);
*/
#endif
