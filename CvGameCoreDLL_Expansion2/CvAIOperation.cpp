			...
			if(pThisArmy->GetNumSlotsFilled() >= 1)
			{
				...
				else if (pCivilian && m_eCurrentState == AI_OPERATION_STATE_RECRUITING_UNITS)
				{
					...
					else
					{
						...
						{
							if (m_eOwner == -1 || GET_PLAYER(m_eOwner).getNumCities() > 1 || GET_PLAYER(m_eOwner).GetDiplomacyAI()->GetBoldness() > 5) // unless we'd rather play it safe
							{
								ArmyInPosition(pThisArmy);
								return true;
							}
	...
	if(iThisFormationIndex != NO_MUFORMATION)
	{
		...
		if(thisFormation)
		{
...
#ifdef AUI_OPERATION_FIND_BEST_FIT_RESERVE_CALCULATE_PERFECT_MATCH_FIRST
			// PERFECT UNIT TYPE ONLY (Both primary and secondary match)
			int iLoop = 0;
			...
			if (pBestUnit != NULL)
			{
				...
				if (GC.getLogging() && GC.getAILogging())
				{
					strMsg.Format("Found perfect unit for muster at x=%d y=%d, target of x=%d y=%d", (pMusterPlot) ? pMusterPlot->getX() : -1, (pMusterPlot) ? pMusterPlot->getY() : -1, (pTargetToUse) ? pTargetToUse->getX() : -1, (pTargetToUse) ? pTargetToUse->getY() : -1);
...
MultiunitFormationTypes CvAIOperationBasicCityAttack::GetFormation() const
{
	...
	return (GC.getGame().getHandicapInfo().GetID() > 3 && !(GC.getMap().GetAIMapHint() & 1)) ? MUFORMATION_BIGGER_CITY_ATTACK_FORCE : MUFORMATION_BASIC_CITY_ATTACK_FORCE;
#else
...
{
#ifdef AMB_DIFFICULTY_ID_ADJUSTMENT
	return (GC.getGame().getHandicapInfo().GetID() > 3 && !(GC.getMap().GetAIMapHint() & 1)) ? MUFORMATION_BIGGER_CITY_ATTACK_FORCE : MUFORMATION_CITY_STATE_ATTACK_FORCE;
#else
							...
							for(int iDY = -iBlastRadius; iDY <= iBlastRadius; iDY++)
							{
								...
								if(pLoopPlot)
								{
									...
									if(ePlotOwner == m_eOwner)
									{
										iThisCityValue -= 1;
#ifdef AUI_OPERATION_TWEAKED_FIND_BEST_TARGET_NUKE
										int iYieldChangeScore = 0;
										for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
										{
											if (pLoopPlot->getYield((YieldTypes)iI) + pkFalloutFeature->getYieldChange(iI) != 0)
											{
												iYieldChangeScore -= MIN(pLoopPlot->getYield((YieldTypes)iI), -pkFalloutFeature->getYieldChange(iI));
											}
										}
...
#ifdef AMB_GETGATHERRANGEFORXUNITS_PLUS_1
	return iRange+1;
#else
	return iRange;
#endif
