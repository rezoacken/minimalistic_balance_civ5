...
function AssignStartingPlots.Create()
	...
	local findStarts = {
		...
		iNumMaxAllowedForRegions = 9,	-- Maximum luxury types allowed to be assigned to regional distribution. CANNOT be reduced below 8!
		iNumTypesAssignedToRegions = 0,
		resourceIDs_assigned_to_regions = {},
		iNumTypesAssignedToCS = 5,		-- Luxury types that will be placed only near city states
...
function AssignStartingPlots:__InitLuxuryWeights()
	...
	{self.whale_ID,		20},					-- Leaning toward types that are used less often by civs.
	{self.pearls_ID,	20},
	{self.gold_ID,		10},					-- Recommended that this list also contains every available luxury.
	{self.silver_ID,	10},
	{self.gems_ID,		20},					-- NOTE: Marble affects Wonders, so is handled as a special case, on the side.
	{self.ivory_ID,		10},
	{self.fur_ID,		15},
	{self.dye_ID,		10},
	{self.spices_ID,	15},
...
function AssignStartingPlots:MeasureStartPlacementFertilityOfPlot(x, y, checkForCoastalLand)
	...
	elseif featureType == FeatureTypes.FEATURE_FLOOD_PLAINS then
		plotFertility = 3; -- Reducing Flood Plains value slightly. -1/26/2011 BT
	else
...
function AssignStartingPlots:PlaceImpactAndRipples(x, y)
	...
	local ripple_values = {97, 95, 92, 89, 79, 69, 59, 49, 39, 30, 24, 15};
...
function AssignStartingPlots:AddStrategicBalanceResources(region_number)
	...
	local even = self.firstRingYIsEven;
	local nextX, nextY, plot_adjustments;
	local iron_list, horse_list, coal_list, oil_list = {}, {}, {}, {};
	local iron_fallback, horse_fallback, coal_fallback, oil_fallback = {}, {}, {}, {};
	local radius = 3;
	...
	local shuf_list;
	local placed_iron, placed_horse, placed_coal, placed_oil = false, false, false, false;

	if table.maxn(iron_list) > 0 then
		shuf_list = GetShuffledCopyOfTable(iron_list)
		iNumLeftToPlace = self:PlaceSpecificNumberOfResources(self.iron_ID, iron_amt, 1, 1, 1, 0, 1, shuf_list);
	...
	if table.maxn(horse_list) > 0 then
		...
		iNumLeftToPlace = self:PlaceSpecificNumberOfResources(self.horse_ID, horse_amt, 1, 1, 1, 0, 1, shuf_list);
		if iNumLeftToPlace == 0 then
			placed_horse = true;
		end
	end
	if table.maxn(oil_list) > 0 then
		...
		iNumLeftToPlace = self:PlaceSpecificNumberOfResources(self.oil_ID, oil_amt, 2, 1, 1, 0, 1, shuf_list);
		if iNumLeftToPlace == 0 then
	...
	if table.maxn(coal_list) > 0 then
		...
		iNumLeftToPlace = self:PlaceSpecificNumberOfResources(self.coal_ID, coal_amt, 1, 1, 1, 0, 1, shuf_list);
		if iNumLeftToPlace == 0 then
			placed_coal = true;
		end
	end
				...
				if impact_table_number == 1 then
					if self.strategicData[plotIndex] == 0 then
						...
						if res_plot:GetResourceType(-1) == -1 then -- Placing this strategic resource in this plot.
							...
							res_plot:SetResourceType(res_ID[use_this_res_index], res_quantity[use_this_res_index]+Map.Rand(3,"Random strategics"));
							print("RANDOM RESSOURCE");
		...
		for loop, plotIndex in ipairs(plot_list) do
			if bCheckImpact == false or impact_table[plotIndex] == 0 then
				...
				if res_plot:GetResourceType(-1) == -1 then -- Placing this resource in this plot.
					if impact_table_number == 1 or quantity > 1 then 
						rand_amt = Map.Rand(3, "Random amount of ressources - Lua");
						print("RAND RESSOURCE", rand_amt, quantity);
					end
					res_plot:SetResourceType(resource_ID, quantity + rand_amt);
					self.amounts_of_resources_placed[resource_ID + 1] = self.amounts_of_resources_placed[resource_ID + 1] + quantity + rand_amt;
				...
				if res_ID == self.whale_ID or res_ID == self.pearls_ID or res_ID == self.crab_ID then
					...
					elseif res_ID == self.crab_ID and self.regionTypes[region_number] == 4 then
						...
						if self.startLocationConditions[region_number][1] == true then -- This region's start is along an ocean, so water-based luxuries are allowed.
							if self.regionTerrainCounts[region_number][8] >= 10 then -- Enough water available.
...
function AssignStartingPlots:GetCityStateLuxuriesTargetNumber()
	...
	local worldsizes = {
		...
		[GameInfo.Worlds.WORLDSIZE_LARGE.ID] = 8,
		[GameInfo.Worlds.WORLDSIZE_HUGE.ID] = 8
		}
...
function AssignStartingPlots:GetRegionLuxuryTargetNumbers()
	...
	--
	local small_values = {0, 3, 3, 3, 4, 4, 4, 3, 3, 3, 2, 2, 2, 2, 1, 1, 1, 1, 1, 1, 1, 1};
	--
...
function AssignStartingPlots:GetWorldLuxuryTargetNumbers()
	...
	elseif self.resource_setting == 3 then -- Abundant
		...
		worldsizes = {
			...
			[GameInfo.Worlds.WORLDSIZE_TINY.ID] = {33, 4},
			[GameInfo.Worlds.WORLDSIZE_SMALL.ID] = {50, 4},
			[GameInfo.Worlds.WORLDSIZE_STANDARD.ID] = {66, 5},
			[GameInfo.Worlds.WORLDSIZE_LARGE.ID] = {83, 5},
			[GameInfo.Worlds.WORLDSIZE_HUGE.ID] = {105, 6}
		}
	end
...
function AssignStartingPlots:PlaceLuxuries()
	...
	for loop, reg_data in ipairs(self.regions_sorted_by_type) do
		...
		if iNumLeftToPlace > 0 then
			...
			RndTable[region_number] = iNumLeftToPlace;
		end
	end
		...
		if self.city_state_validity_table[city_state] == false then
			...
			if iNumCSAllowed > 0 then
				for loop, res_ID in ipairs(cs_only_types) do
					lux_possible_for_cs[res_ID] = 90 / iNumCSAllowed;
				end
			end
			...
			if self.iNumTypesRandom > 0 or region_number > 0 then
				...
				if region_number > 0 then
					...
					if allowed_luxuries[res_ID] == true then
						lux_possible_for_cs[res_ID] = 10 / iNumAllowed;
					end
				end
			...
			if iNumAvailableTypes == 0 then
				...
				luxury_plot_lists = self:GenerateLuxuryPlotListsAtCitySite(x, y, 2, false)
				shuf_list = GetShuffledCopyOfTable(luxury_plot_lists[primary])
				local iNumLeftToPlace = self:PlaceSpecificNumberOfResources(use_this_ID, 1, 1, 0.6, -1, 0, 0, shuf_list);
				if iNumLeftToPlace > 0 and secondary > 0 then
					shuf_list = GetShuffledCopyOfTable(luxury_plot_lists[secondary])
					iNumLeftToPlace = self:PlaceSpecificNumberOfResources(use_this_ID, 1, 1, 0.8, -1, 0, 0, shuf_list);
				end
				if iNumLeftToPlace > 0 and tertiary > 0 then
					shuf_list = GetShuffledCopyOfTable(luxury_plot_lists[tertiary])
	...
	if self.iNumTypesRandom > 0 then
		...
		for loop, res_ID in ipairs(self.resourceIDs_assigned_to_random) do
			...
			if self.iNumTypesRandom * 4 > iNumRandomLuxTarget then
				iNumThisLuxToPlace = 4 + Map.Rand(3, "");
			...
			--iNumThisLuxToPlace = 4;
			-- Place this luxury type.
			current_list = self.global_luxury_plot_lists[primary];
			iNumLeftToPlace = self:PlaceSpecificNumberOfResources(res_ID, 1, iNumThisLuxToPlace, 0.5, 2, 2, 0, current_list);
			if iNumLeftToPlace > 0 and secondary > 0 then
				current_list = self.global_luxury_plot_lists[secondary];
				iNumLeftToPlace = self:PlaceSpecificNumberOfResources(res_ID, 1, iNumLeftToPlace, 0.5, 2, 2, 0, current_list);
			end
			if iNumLeftToPlace > 0 and tertiary > 0 then
				current_list = self.global_luxury_plot_lists[tertiary];
				iNumLeftToPlace = self:PlaceSpecificNumberOfResources(res_ID, 1, iNumLeftToPlace, 0.75, 2, 2, 0, current_list);
			end
			if iNumLeftToPlace > 0 and quaternary > 0 then
				current_list = self.global_luxury_plot_lists[quaternary];
				iNumLeftToPlace = self:PlaceSpecificNumberOfResources(res_ID, 1, iNumLeftToPlace, 1, 2, 2, 0, current_list);
			end
...
function AssignStartingPlots:PlaceOilInTheSea()
	...
	self:PlaceSpecificNumberOfResources(self.oil_ID, sea_oil_amt, iNumToPlace, 0.3, -1, 2, 5, self.coast_list)
end
------------------------------------------------------------------------------
...
function AssignStartingPlots:GetMajorStrategicResourceQuantityValues()
	...
	local uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt = 2, 2, 5, 4, 5, 6;
	-- Check the resource setting.
	if self.resource_setting == 1 then -- Sparse
		uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt = 1, 3, 4, 3, 4, 4;
	elseif self.resource_setting == 3 then -- Abundant
		uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt = 2, 4, 7, 7, 8, 8;
	end
...
function AssignStartingPlots:PlaceStrategicAndBonusResources()
	...
	local resources_to_place = {
	{self.oil_ID, oil_amt, 65, 1, 3},
	{self.uranium_ID, uran_amt, 35, 3, 0} };
	self:ProcessResourceList(7, 1, self.marsh_list, resources_to_place)

	local resources_to_place = {
	{self.oil_ID, oil_amt, 25, 1, 3},
	{self.aluminum_ID, alum_amt, 50, 1, 2},
	{self.iron_ID, iron_amt, 25, 1, 2} };
	self:ProcessResourceList(13, 1, self.tundra_flat_no_feature, resources_to_place)
	...
	local resources_to_place = {
	{self.oil_ID, oil_amt, 70, 0, 1},
	{self.iron_ID, iron_amt, 30, 1, 1} };
	self:ProcessResourceList(11, 1, self.desert_flat_no_feature, resources_to_place)

	local resources_to_place = {
	{self.iron_ID, iron_amt, 40, 0, 1},
	{self.coal_ID, coal_amt, 20, 1, 2},
	{self.aluminum_ID, alum_amt, 40, 1, 2} };
	self:ProcessResourceList(14, 1, self.hills_list, resources_to_place)

	local resources_to_place = {
	{self.coal_ID, coal_amt, 30, 1, 2},
	{self.uranium_ID, uran_amt, 70, 3, 0} };
	self:ProcessResourceList(33, 1, self.jungle_flat_list, resources_to_place)
	
	local resources_to_place = {
	{self.coal_ID, coal_amt, 35, 1, 2},
	{self.oil_ID, oil_amt, 15, 1, 5},
	{self.uranium_ID, uran_amt, 50, 3, 0} };
	self:ProcessResourceList(35, 1, self.forest_flat_list, resources_to_place)

	local resources_to_place = {
	{self.horse_ID, horse_amt, 100, 1, 2} };
	self:ProcessResourceList(18, 1, self.dry_grass_flat_no_feature, resources_to_place)
	
	local resources_to_place = {
	{self.iron_ID, iron_amt, 35, 2, 3},
	{self.horse_ID, horse_amt, 65, 2, 3} };
	self:ProcessResourceList(13, 1, self.plains_flat_no_feature, resources_to_place)
	...
	self:ProcessResourceList(25 * bonus_multiplier, 3, self.dry_grass_flat_no_feature, resources_to_place)
